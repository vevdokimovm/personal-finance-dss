...
# Резервная пороговая ставка фильтра. Канон §10.2 требует динамики
# r_bench = r_key·(1 − НДФЛ); это значение — ПОСЛЕДНИЙ фолбэк каскада
# (сеть → память → кэш БД → сюда), когда ключевую ставку получить нельзя.
# 🔴 Живёт ровно в одном месте: россыпь литералов 0.14 по семи модулям делала
# правку «в одном месте» физически невозможной — правка одного создавала
# расхождение вместо исправления (ДК-36). Это параметр канона, а не настройка
# окружения, поэтому дом у него здесь, рядом с самим фильтром, а не в .env.
R_BENCH_FALLBACK = 0.14
