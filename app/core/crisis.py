"""
Кризисный модуль: план действий при отрицательном ресурсе.

Действующая редакция модели — `docs/math_model.md` (имя файла стабильно), история
калибровок — `docs/model/model_history.md`. 🔴 Номер редакции здесь НЕ повторяется:
прежняя докстрока помечала модуль как «модель v3.1.0» и осталась верной ровно до
следующей калибровки, после чего утверждала неправду молча.
