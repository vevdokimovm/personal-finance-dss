...
def build_debt_amortization_schedule(
    ...
    *,
    ...
    return DebtScheduleSummary(
        ...
        baseline_total_interest=round(b_interest, 2),
        accelerated_total_interest=round(a_interest, 2),
        interest_saved=round(b_interest - a_interest, 2),
...
def _simulate(
    ...
    *,
    ...
    """
    balances = {o["id"]: float(o["amount"]) for o in obligations}
    rates_m = {o["id"]: float(o.get("interest_rate", 0)) / 12 for o in obligations}
    payments = {o["id"]: float(o.get("monthly_payment", 0)) for o in obligations}
    ...
    while any(balances[i] > 0.01 for i in all_ids) and month < horizon_months:
        month += 1
        freed = 0.0
        for i in all_ids:
            if balances[i] <= 0:
                continue
            interest = balances[i] * rates_m[i]
        ...
        pool = extra_monthly + (freed if cascade_freed else 0.0)
        if pool > 0:
            for i in target_order:
                if pool <= 0:
                    break
                if balances.get(i, 0) <= 0:
                    continue
                apply = min(pool, balances[i])
                balances[i] -= apply
                pool -= apply

    capped = month >= horizon_months and any(balances[i] > 0.01 for i in all_ids)
