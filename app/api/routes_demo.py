...
# 🔴 Переписано в v8.46.0. Прежний текст был написан для разработчика, а читает его
# человек, который впервые видит продукт: «Avalanche», «Fail-loud», «BLR», «Bliq», «ПДН»,
# «runway» — прямая нотация модели, запрещённая [CMP-05]. Плюс девять полей из двадцати
# имели склейку слов без пробела («несколько целей.Свободный поток»): первый экран
# выглядел так, будто текст сгенерировали и не прочитали (design-critic).
#
# Числа — по канону денег: неразрывный пробел в разрядах и перед знаком, запятая
# как десятичный разделитель (канон денег проекта). Точка в «27.9%» читалась
# бухгалтером как ошибка данных, а не как опечатка вёрстки.
#
# `accent` — ТОЛЬКО четыре значения, которые понимает фронт (amber | green | red | blue).
# Было ещё `cyan`, `violet`, `slate` — четыре портрета из десяти молча теряли цвет ярлыка
# и выглядели недоделанными: поле типизировано как `str`, поэтому ни mypy, ни TS не спорили.
CASE_META: dict[str, dict[str, str]] = {
    "anna": {
        ...
        "accent": "amber",
        "situation": (
            "Доход 180 000 ₽, ипотека, автокредит и рассрочка, несколько целей. Свободные деньги "
            "после платежей остаются, но нагрузка ощутимая."
        ),
        "expect": (
            "Сначала гасим самый дорогой кредит, запас на чёрный день держим не ниже нормы, "
            "остаток — на цели по важности. Прогноз почти ровный: зарплата стабильная."
        ),
    },
    "dmitriy": {
        "n": "2",
        "name": "Дмитрий, 28",
        "role": "IT-разработчик · Санкт-Петербург",
        "tag": "Здоровый бюджет",
        "accent": "green",
        "situation": "Высокий растущий доход (зарплата и фриланс), дешёвая IT-ипотека, накопления.",
        "expect": (
            "Упор на цели и рост: дешёвый кредит досрочно гасить невыгодно, деньги полезнее в "
            "другом месте. Прогноз уверенно растёт вслед за доходом."
        ),
    },
    "mikhail": {
        "n": "3",
        ...
        "accent": "red",
        "situation": (
            "Четыре кредита, расходы вместе с платежами съедают почти весь доход, и доход падает."
        ),
        "expect": (
            "Здесь система честно откажется советовать распределение и покажет разбор ситуации: "
            "свободных денег нет. Прогноз снижается вслед за доходом."
        ),
    },
    "igor": {
        "n": "4",
        "name": "Игорь, 25",
        "role": "Junior-разработчик · Нижний Новгород",
        "tag": "Старт карьеры",
        "accent": "blue",
        "situation": "Невысокий, но растущий доход, кредитов нет, накоплений почти нет.",
        "expect": (
            "Сначала запас на чёрный день — он ниже нормы, — потом цели, которые поднимут доход. "
            "Прогноз плавно растёт."
        ),
    },
    "olga": {
        "n": "5",
        "name": "Ольга, 38",
        "role": "Библиотекарь · Тула",
        "tag": "Микс-стратегия",
        "accent": "blue",
        "situation": (
            "Умеренный доход с алиментами, один кредит, часть запаса накоплена, цели близкие по "
            "сроку."
        ),
        "expect": (
            "Деньги делятся понемногу на все три направления сразу: ни одно не выигрывает с "
            "большим отрывом."
        ),
    },
    "viktor": {
        "n": "6",
        "name": "Виктор, 58",
        "role": "Главный инженер · Екатеринбург",
        "tag": "Пред-пенсионный",
        "accent": "green",
        "situation": "Стабильный доход, крупный депозит (850 000 ₽), цели близкие по сроку.",
        "expect": (
            "Ликвидность в приоритете; близкие цели закрываются разом из накоплений, а не из "
            "месячного потока. Прогноз растёт за счёт процентов по депозиту."
        ),
    },
    "ekaterina": {
        "n": "7",
        "name": "Екатерина, 32",
        "role": "Дизайнер на проектах · Сочи",
        "tag": "Волатильный доход",
        "accent": "amber",
        "situation": (
            "Нерегулярный доход (проекты, роялти, консультации), рассрочка под 0 % и дорогая "
            "кредитка, четыре цели, небольшой буфер."
        ),
        "expect": (
            "Упор на запас: при нерегулярном доходе он важнее скорости. Кредитку гасим первой как "
            "самую дорогую. Прогноз колеблется — коридор широкий."
        ),
    },
    "artyom": {
        "n": "8",
        "name": "Артём, 34",
        "role": "Тимлид в финтехе · Москва",
        "tag": "Агрессивный",
        "accent": "green",
        "situation": (
            "Высокий растущий доход, три долга (кредитка 27,9 %, автокредит 15,5 %, ипотека "
            "7,8 %), инвестиции и брокерский счёт."
        ),
        "expect": (
            "Долги выстраиваются по ставке, дешёвую ипотеку система не трогает. Профиль "
            "агрессивный — вес роста высокий. Прогноз круто растёт."
        ),
    },
    "natalya": {
        "n": "9",
        "name": "Наталья, 61",
        "role": "Пенсионерка с подработкой · Самара",
        "tag": "Ликвидная позиция",
        "accent": "blue",
        "situation": (
            "Пенсия и подработка, долгов нет, вклад 1,25 млн и счёт, две близкие цели и одна "
            "бессрочная."
        ),
        "expect": (
            "Близкие цели закрываются разом из вклада, а не из месячного потока. Прогноз растёт за "
            "счёт процентов по крупному депозиту."
        ),
    },
    "pavel": {
        "n": "10",
        "name": "Павел, 39",
        "role": "Прораб, сезонный доход · Тюмень",
        "tag": "Перегруз по долгам",
        "accent": "red",
        "situation": (
            "Сезонный доход (зимой мало), четыре долга включая микрозаём под 36,5 %, в межсезонье "
            "денег не хватает."
        ),
        "expect": (
            "Включается разбор для дефицита: что пересмотреть, что реструктурировать и на сколько "
            "месяцев хватит запаса. Прогноз сезонный — заметная волна."
        ),
    },
}
...
class DemoCase(BaseModel):
    ...
    key: str = Field(description="Ключ кейса для `/demo/load?case=`.")
    n: str = Field(description="Порядковый номер в списке (строкой, как в разметке).")
    name: str = Field(description="Имя и возраст: «Анна Петрова, 36».")
    role: str = Field(description="Занятие и город: «Маркетолог · Москва».")
    tag: str = Field(description="Ярлык ситуации: «Пограничный», «Критический».")
    accent: str = Field(description="Семантический цвет ярлыка: amber | green | red | blue.")
    situation: str = Field(description="Исходные данные портрета человеческим языком.")
    expect: str = Field(description="Что покажет алгоритм и почему — до нажатия.")
...
class DemoCases(BaseModel):
    ...
    cases: list[DemoCase]
    keys: list[str] = Field(description="Только ключи — для мест, где метаданные не нужны.")


@router.get("/demo/cases", summary="Список демо-кейсов с метаданными")
def list_cases() -> DemoCases:
    return DemoCases(
        cases=[DemoCase(key=k, **CASE_META[k]) for k in CASES],
