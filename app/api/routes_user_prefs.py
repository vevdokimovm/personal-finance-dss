...
@router.get("", response_model=UserPrefsResponse, summary="Параметры пользователя U (Lmin, R, H, r_bench)")
def read_prefs(db: Session = Depends(get_db)) -> UserPrefsResponse:
    return get_user_prefs(db)


@router.patch("", response_model=UserPrefsResponse, summary="Обновить параметры пользователя")
def patch_prefs(payload: UserPrefsUpdate, db: Session = Depends(get_db)) -> UserPrefsResponse:
