...
def create_budget(
    ...
    `category` уникальна в паре с владельцем — `UniqueConstraint("user_id", "category")`,
    миграция 0036. До неё ключ был глобальным, и второй пользователь, заводящий «Продукты»,
    получал 500; описание «уникальна глобально» осталось здесь на версию дольше правки.

    Существующая строка ищется БЕЗ фильтра по `is_deleted`: иначе повторное создание
    категории после мягкого удаления упёрлось бы в UNIQUE-конфликт со старой удалённой
    строкой вместо того, чтобы её оживить. Оживление и есть корректный смысл upsert (P1.7).

    🔴 **Владение (`household_id`) upsert НЕ меняет** — нашёл `/code-review ultra`.
    Ветка обновления правила лимит и молча теряла `household_id` из тела: человек
    отправлял личный бюджет с `household_id`, чтобы сделать его семейным, получал 201 —
    и бюджет оставался личным. Ровно тот класс, против которого заводился
    `ensure_can_share` (v8.55.0): гард закрыл путь INSERT и не закрыл путь UPDATE.

    Различаются два случая, и они не равны:

    - `household_id is None` — форма при правке поле не шлёт вовсе (`BudgetForm.tsx`),
      значит это «не трогай владение», а не «сделай личным». Прежнее владение сохраняется.
    - `household_id` задан и отличается от текущего — запрошена **смена владения**,
      операция, которой продукт не делает нигде. Ответ 409: запрос не применён, и видно
      почему. Тихо разрешить переезд через POST значило бы завести полускрытый способ
      делать объявленное неподдерживаемым, работающий только для бюджетов.

    Смену владения отклоняет РОУТЕР (`_guards.ensure_scope_unchanged`), а не эта
    функция: HTTP-код — не дело слоя доступа к данным, и проверка стоит там же,
    где соседняя `ensure_can_share`, чтобы обе было видно вместе.
    """
