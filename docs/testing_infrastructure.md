...
в сумме = 1 и т.д. Инварианты сверяются строго по `docs/math_model_v3_5_0.md`. Это самый
...
> **Легенда колонки «Тир».** `fast`/`full`/`deep`/`e2e` — тир прогона, к которому относится тестовый
> файл. Прочерк `—` означает **«не применимо»**: строка описывает не тест, а инфраструктуру/конфиг
> (нагрузочный сценарий locust, YAML CI, Makefile-цели, конфиги pytest/coverage), у которого тира по
> определению нет. `—` НЕ означает «тест отсутствует» — все перечисленные выше виды тестов в проекте
> существуют и зелёные.
>
> **Где тонко (честный статус, не дыра).** Тиры `full` и `deep` — рабочий каркас минимальной ширины
> (`visual_regression` и `a11y_axe` — по 1 тест-функции, `property_stress` — 2). Это не заглушки
> (внутри реальная логика), но точка осознанного роста: при необходимости расширяются скриншотами по
> каждой ключевой странице в обеих темах, дополнительными a11y-сценариями и стресс-инвариантами.
