...
**Состояние каналов (свои замеры 16.09.2026):** `mcp__exa__web_search_exa` и
`mcp__exa__web_fetch_exa` — работают. Wayback `web.archive.org` — **HTTP 302** (не 429, но на
снимки не опирался). OpenAlex — HTTP 200. Unpaywall — HTTP 200. `curl` с браузерным UA — работает
(им взят PDF на 476 КБ в соседнем файле темы). 🔴 **Замер против антибота:** страница
`scholar.law.colorado.edu/cgi/viewcontent.cgi` в прошлом заходе отдавала **403 на `curl`** и
**228-байтную пустышку на `r.jina.ai`**; `mcp__exa__web_fetch_exa` по тому же URL вернул
**60 228 байт разобранного текста статьи**. Это прямой замер того, ради чего батч и заведён.

Взяты только пункты, остававшиеся открытыми по последнему упоминанию (§Д5.4, §П4.3 «НЕ ДОБЫТО»,
§П4.5, §П4.6, §П4.4 D). Закрытое ранее (AI Act, DSA, CFPB, Роскомнадзор № 149, Hanna et al.,
Jackman & Kanerva, Meyer et al. 2019, FCA через Д14) не переоткрывалось.

## Г30.1-11. 🔴 Polonetsky, Tene & Jerome (2015) — ДОБЫТА (был отказ по пяти каналам)

Было (§П4.3 «НЕ ДОБЫТО»): «scholar.law.colorado.edu — curl 403 (Cloudflare), r.jina.ai — 200,
228 байт пустышка, Wayback — 404, archive.org API — 429, SSRN — 403 дважды. Цитат из статьи
не привожу.»

**Взято через Exa:** `mcp__exa__web_fetch_exa` →
`https://scholar.law.colorado.edu/cgi/viewcontent.cgi?article=1195&context=ctlj`
— **разобрано 60 228 байт текста**, 16.09.2026. Реквизиты подтверждены по титулу самого файла:
Polonetsky J., Tene O., Jerome J. «Beyond the Common Rule: Ethical Structures for Data Research
in Non-Academic Settings», **13 COLO. TECH. L.J. 333 (2015)**, Vol. 13, Issue 2, Article 7;
авторы — Future of Privacy Forum (Polonetsky — Co-chair and Executive Director, Tene — Senior
Fellow, Jerome — Policy Counsel).

**Оглавление (дословно, с номерами страниц):** ABSTRACT 333 · INTRODUCTION 334 · I. BIG DATA
BENEFITS — AND RISKS 337 · II. CONTEXTUAL AND NON-CONTEXTUAL DATA USE 339 · III. WHAT SHOULD BE
COVERED BY A CSRB 341 (A. Data experimentation 341; B. Non-contextual data uses 346; C. Disparate
impact 349) · IV. CSRB STRUCTURE 352 (A. Who is covered 352; B. External (FTC-approved) or Internal
(Organization-led) 353) · V. GUIDELINES FOR CSRB ENGAGEMENT 356 (A. Guiding Substantive Principles
356: 1. Respect for persons 357; 2. Beneficence 358; 3. Justice 360; 4. Respect for law 361;
B. Promoting Independence and Trust 362; C. Documentation and Accountability 365) · CONCLUSION 366.

**Абстракт — дословно, предмет статьи:**

> «This article addresses **the processes required to authorize non-contextual data uses at
> corporations or not-for-profit organizations in the absence of additional notice and choice.**
> Although many of these challenges are also relevant to academic researchers, their work will
> often be guided by the oversight of Internal Review Boards (which are required for many — but not
> all — new research uses of personal information).»

**Введение — почему нерегулируемое исследование всё равно нуждается в правилах (дословно):**

> «Such research initiatives, which transcend the scope of activities subject to existing ethical
> frameworks, **should be subject to clear principles and guidelines. An uncertain regulatory
> terrain could jeopardize the value of important research, which may be perceived as ethically
> tainted or become hidden from the public domain to prevent scrutiny.**»

**§IV.B — главный для нас ответ: внешний совет или внутренний (дословно):**

> «A threshold question is whether a CSRB should be an internal corporate organ or an external body.
> On the one hand, **it would be challenging to create an internal process with sufficient
> accountability and transparency to make credible decisions that assuage public concerns.** On the
> other hand, it would be equally hard for organizations to hand over a high volume of strictly
> confidential, sometimes highly technical business decisions to an external decision-making body.»
>
> «This article recommends that **any legislative effort to establish CSRBs allow for the creation
> of both external and internal review boards** … **A dual-track approach that permits both options
> would ensure that CSRBs are scalable to address the needs of both small and large companies and
> organizations.**»
>
> «An external CSRB would be **a reasonable option for an organization that lacks the resources,
> ability, or expertise to develop methodical internal processes.** These bodies … **could serve
> multiple companies in an industry or sector, thus solving the problem of small- and medium-size
> enterprises that lack the necessary scale to create an internal CSRB.** … Undergoing an external
> review process would offer an organization **a presumption of legitimacy** in cases of challenges
> to the fairness of data research or non-contextual uses of personal information.»
>
> Ограничение внешнего варианта (дословно): «one significant benefit a CSRB could offer is
> **ongoing monitoring of and consultation about non-contextual data uses over time.** In contrast
> to an internal organ, **external CSRBs will likely lack the ability — or capacity — for such
> ongoing review.** Furthermore, with ethical reviews being made in a virtual vacuum, specific
> decisions may not reflect the full spectrum of risks and rewards…»

🔴 **Что это меняет для §Д5.6 и вывода П4.3 «минимальный процесс одобрения».** Прежде вывод об
отраслевой норме строился на Jackman & Kanerva (внутренний процесс Facebook) плюс косвенной ссылке
на эту статью. Теперь первоисточник прямо признаёт: **для компании нашего размера внутренний совет
нереалистичен по ресурсу, и авторы предлагают внешний/отраслевой как штатный путь для СМП.** Для
FINPILOT это переводит вопрос из «завести свой этический комитет» в «зафиксировать письменный
регламент + при спорных экспериментах привлекать внешнее заключение». Плюс их же оговорка
объясняет, почему внешний путь не заменяет внутренний целиком: **непрерывный мониторинг внешний
совет не тянет.**

⬜ **Честная граница выборки:** Exa вернула текст **до §IV.B включительно** (60 228 байт — лимит
выборки, не конец статьи). **§V «Guidelines for CSRB Engagement» (с. 356–366) — четыре
субстантивных принципа, независимость и документирование — в этот заход не извлечён.** Это
единственный остаток пункта; канал рабочий, добирается повторным запросом.

## Г30.1-12. Freedman 1987 — полный текст по-прежнему закрыт, но ДОБЫТЫ ДОСЛОВНЫЕ ЦИТАТЫ ИЗ ТЕЛА СТАТЬИ, включая правило остановки

Было (§П4.5): «НЕ ДОБЫТ… Дословная формулировка "обязанности прекратить" со страницей не получена;
цитирую только абстракт и вторичный разбор. Без полного текста нельзя исключить, что в теле статьи
есть отдельная формулировка про остановку испытаний.»

**Перепроверка каналов 16.09.2026:** NEJM `doi/full/10.1056/NEJM198707163170304` — только абстракт,
полный текст за подпиской; Georgetown Repository `handle/10822/730146` — метаданные и KIE-реферат,
файла нет; Exa-поиск по формулировкам тела статьи полного текста не дал. **Полный текст — закрыт.**

**Но Exa нашла открытые источники, цитирующие тело статьи ДОСЛОВНО С НОМЕРОМ СТРАНИЦЫ.**

1) **Определение клинического равновесия — с. 144** (Springer, *Encyclopedia of Global Bioethics*,
статья «Clinical Equipoise», Horn A. R., Weijer C., 2016, DOI 10.1007/978-3-319-09483-0_84,
через Exa). Дословно: «According to Freedman, a state of clinical equipoise obtains when there
exists **"an honest, professional disagreement among expert clinicians about the preferred
treatment"** (Freedman, *New England Journal of Medicine*, 317(3), **144**, 1987).»

2) 🔴 **Вторая часть определения — то самое правило про дизайн и исход спора** (BMJ / открытая копия
PMC: «Clinical equipoise and not the uncertainty principle is the moral underpinning of the
randomised controlled trial», `https://pmc.ncbi.nlm.nih.gov/articles/PMC1127868/`, снято через Exa
16.09.2026; ссылка [1-3] у авторов = Freedman 1987). Дословная цитата Freedman:

> «**The second part of clinical equipoise states: "the trial must be designed in such a way as to
> make it reasonable to expect that, if it is successfully concluded, clinical equipoise will be
> disturbed. In other words, the results of a successful trial should be convincing enough to
> resolve the dispute among clinicians."**»

3) **Почему личное предпочтение исследователя не имеет нормативного веса** (там же, дословная
цитата Freedman):

> «the ethics of medical practice **grant no ethical or normative meaning to a treatment preference,
> however powerful, that is based on a hunch or anything less than evidence publicly presented and
> convincing to the clinical community.** Persons are licensed as physicians after they demonstrate
> the acquisition of this professionally validated knowledge, **not after they reveal a superior
> capacity for guessing.**»

4) И следствие для дизайна (там же): «**It would have physicians respect the fact that "their less
favoured treatment is preferred by colleagues whom they consider to be responsible and competent."**»

🔴 **Это ПОДТВЕРЖДАЕТ и уточняет правку §П4.5 к своду раздела 5 п. 3.** Догадка П4.5 («условие
прекращения — исчезновение неопределённости в сообществе, а не появление предпочтения у
исследователя») теперь стоит на дословных цитатах тела статьи, а не на реконструкции из абстракта.
Более того, найдена формулировка, которой в файле не было вовсе: **критерий успеха испытания — что
его результат УБЕДИТЕЛЕН настолько, чтобы разрешить спор**, а не что точечная оценка разошлась.
Перенос на нас (правка Д5.6 п. 8) остаётся в силе и усиливается: **exploration выключается по
заранее заданному статистическому критерию, дающему убедительный, а не любой, перевес.**
Оговорка снимается частично: остаётся неизвестным, есть ли в теле статьи ОТДЕЛЬНЫЙ пассаж про
досрочную остановку уже идущего испытания (цитаты выше — про условия начала и про дизайн).

## Г30.1-13. Meyer (2015) «Two Cheers for Corporate Experimentation» — ДОБЫТА (была «не искалась отдельно»)

Было (§П4.3 «НЕ ДОБЫТО»): «Meyer 2015 "Two cheers for corporate experimentation" — не искался
отдельно (бюджет участка)»; в §Д.3 (п. 3247) значилось «сам текст 2015 г. не добыт».

**Взято через Exa:** `mcp__exa__web_fetch_exa` →
`https://scholar.law.colorado.edu/cgi/viewcontent.cgi?article=1194&context=ctlj` (16.09.2026).
Реквизиты по титулу: **Meyer M. N. «Two Cheers for Corporate Experimentation: The A/B Illusion and
the Virtues of Data-Driven Innovation», 13 COLO. TECH. L.J. 273 (2015)**, Vol. 13, Issue 2,
Article 6. Автор на момент публикации — J.D., Ph.D., Assistant Professor and Director of Bioethics
Policy, Union Graduate College — Icahn School of Medicine at Mount Sinai Bioethics Program.

**Оглавление — дословно, со страницами** (важно: оно само по себе отвечает на вопрос участка
«из чего состоит аргумент про A/B illusion»):

> INTRODUCTION: TWO FRAMES FOR THINKING ABOUT CORPORATE EXPERIMENTATION … 274
> I. BACKGROUND TO THE FACEBOOK EXPERIMENT … 279 (A. Facebook's Algorithmic News Feed Practice 279;
> B. The Emotional Valence of News Feed Posts 282; C. Four Hypotheses about the Effects of News Feed
> 283; D. The Facebook "Emotional Contagion" Experiment 285)
> II. FRAME ONE: HUMAN SUBJECTS RESEARCH … 287 (**A. Subjects Gave No Ethically Meaningful Consent
> to the Experiment 288; B. Why Consent Is Not Always an Ethical Requirement of Human Subjects
> Research 292; C. Was Informed Consent an Ethical Requirement of the Facebook Experiment? 298**:
> 1. Infeasibility 298; 2. Incompletely Informed Consent 298; 3. Debriefing 300;
> **4. Minimal Risk Analysis 302**)
> III. FRAME TWO: RESPONSIBLE INNOVATION … 310 (A. The Ethical Relevance of the Distribution of
> Research Risks and Benefits 310; **B. Mini Case Study in the A/B Illusion: OkCupid's Matching
> Algorithm Experiment 312; C. How the Research/Practice Distinction Fosters the A/B Illusion (And
> Overprotects Subjects and Underprotects Users) 321**; D. A Brief Note on the Problem of Unethical
> Underlying Practices 324)
> CONCLUSION: **RESPONSIBLE INNOVATION & A CULTURE OF CONTINUOUS TESTING** … 327

🔴 **Для нас содержательно важны две вещи, видимые уже по структуре и подтверждающие §C (Meyer et
al. 2019 PNAS).** (1) Автор строит аргумент не «эксперименты этичны/неэтичны», а **«A/B illusion»:
непротестированное внедрение считается нормой, а протестированное — нарушением, хотя риск у первого
выше** (§III.C — «Overprotects Subjects and Underprotects Users»). (2) Отдельный подраздел
**«Minimal Risk Analysis» (с. 302)** — это готовая рамка порога, ниже которого отдельное согласие
не требуется; именно её не хватало нашему §Д5.6 при формулировке правила «когда A/B у нас требует
явного уведомления». Полный текст доступен по тому же каналу; в этот заход извлечены оглавление,
титул и введение (лимит выборки) — тело §II.C.4 и §III.C добирается повторным запросом.

## Г30.1-14. Кодекс этики в сфере ИИ (Альянс ИИ, 2021) — ДОБЫТ ПОЛНЫЙ ТЕКСТ (был «URL известен, не качал»)

Было (§П4.4 C, п. 3531): «также base.garant.ru/406862712, **сам текст в этом заходе не качал** —
бюджет ушёл на Кодекс ЦБ. НЕ ДОБЫТО.»

**Канал:** `curl -sk --http1.1` с браузерным UA →
`https://ethics.a-ai.ru/assets/ethics_files/2023/05/12/Кодекс_этики_20_10_1.pdf`
— **HTTP 200, 3 095 695 байт, `application/pdf`**; `pdftotext -layout` → **51 831 байт текста**.
Снято 16.09.2026. (Exa для этого не понадобилась — обычный `curl` прошёл; фиксирую канал честно.)

**Пункты, относящиеся к нашему вопросу — дословно.**

П. 1.5 «Оценка рисков и гуманитарного воздействия»:
> «Акторам ИИ **рекомендуется проводить оценку потенциальных рисков применения СИИ**, включая
> социальные последствия для человека, общества и государства, гуманитарного воздействия СИИ на
> права и свободы человека на разных стадиях ее жизненного цикла, **в том числе при формировании и
> использовании наборов данных; осуществлять долгосрочный мониторинг проявления таких рисков**;
> учитывать сложность поведения СИИ, включая взаимосвязь и взаимозависимость процессов в жизненном
> цикле СИИ при оценке рисков.
> **Для критических приложений СИИ в особых случаях приветствуется проведение оценки рисков
> посредством привлечения нейтральной третьей стороны или уполномоченного официального органа**,
> но без ущерба для работоспособности и информационной безопасности такой СИИ, а также охраны
> интеллектуальной собственности…»

П. 2.1 «Риск-ориентированный подход»:
> «**Уровень внимания к этическим вопросам в области ИИ и характер соответствующих действий Акторов
> ИИ должен быть пропорционален оценке уровня рисков**, создаваемых конкретными технологиями и СИИ
> для интересов человека и общества. **Оценка уровня рисков учитывает как известные, так и
> возможные риски, при этом принимается во внимание как уровень вероятности угроз, так и их
> возможный масштаб в краткосрочной и долгосрочной перспективе.** … Во исполнение настоящего Кодекса
> **рекомендуется разработка и использование методики оценки рисков СИИ.**»

П. 2.4 «Непричинение вреда»:
> «Любое использование, **в том числе проектирование, разработка, тестирование, внедрение,
> эксплуатация СИИ**, способных целенаправленно причинять вред окружающей среде, жизни и (или)
> здоровью человека, имуществу граждан и юридических лиц, недопустимо.»

П. 2.5 «Идентификация ИИ в общении с человеком»:
> «Акторам ИИ **рекомендуется осуществлять добросовестное информирование пользователей об их
> взаимодействии с СИИ**, когда это затрагивает вопросы прав человека и критических [сфер]…»

П. 1.2 «Уважение автономии и свободы воли человека»:
> «Акторы ИИ должны принимать необходимые меры, направленные на **сохранение автономии и свободы
> воли человека в принятии им решений, права выбора**…»

🔴 **Что это закрывает.** Российский контур §П4.4 C («как решают этическую сторону — в технических
публикациях НИКАК») теперь имеет письменный ориентир, пусть и мягкого права: **Кодекс прямо
распространяет требование непричинения вреда на стадию ТЕСТИРОВАНИЯ** (п. 2.4) — то есть
A/B-эксперимент внутри СИИ формально попадает под его действие; **пропорциональность усилий уровню
риска** (п. 2.1) даёт нам основание для градации «низкорисковый эксперимент — внутренний регламент,
высокорисковый — внешняя оценка» (п. 1.5), что совпадает с дуальной схемой Polonetsky et al.
(Г30.1-11). Статус документа не меняется: **добровольное присоединение, не нормативный акт**;
как обязанность для нас он не действует и в тексте темы должен подаваться именно так.

## Г30.1-15. 🔴 Российская практика: ВТБ-публикация с методологией НАЙДЕНА (была «не найдена»), и она прямо упирается в off-policy проблему

Было (§П4.4 D): «**ВТБ: публикация с методологией** — профиль habr.com/ru/users/VTB/posts/ отдал
200/74 203 байта и "здесь пока нет ни одной публикации"… по выдаче — только подкаст (аудио)».
И там же: «**Off-policy оценка (IPS/DR) в продакшене российского банка — не найдено ни одного
описания.**»

**Exa нашла публикацию по СМЫСЛУ, а не по имени компании** (обычный поиск её не отдавал):
Тимохин Д., Сизов В., Лукашевич А., Суравейкин Е. (команда ЖЦК, машинное обучение в ВТБ).
«Многорукие бандиты: когда классическое тестирование не работает». Хабр, блог компании ВТБ,
**05.06.2025**, `https://habr.com/ru/companies/vtb/articles/915698/` —
`mcp__exa__web_fetch_exa`, разобрано, снято 16.09.2026.

**🔴 Числа, которых в файле не было (дословно):**

> «участвуют в тестировании порой совсем небольшие группы людей: бывает так, что **из ста тысяч
> клиентов только двести двадцать человек как-то отреагируют на предложение**. Такова реальность
> продукта. **Конверсия в 0,22%** может быть репрезентативной…»

> «Контекстные многорукие бандиты не просто работали лучше классического тестирования… **Конверсия
> в тестовой среде выросла более чем в три раза, а объем привлеченных средств увеличился на 42%**
> по сравнению с другими методами… **Пока мы ещё не запускали многоруких бандитов в полноценный
> пилот.**»

**🔴 Прямая формулировка off-policy проблемы — дословно:**

> «представьте, что вы хотите протестировать алгоритмы многоруких бандитов и у вас даже есть
> выборка с клиентами, руками и ответами. Возникает следующая проблема: **в какой-то момент бандит
> выдаст руку, ответа по которой нет. Фактически, получили базовую проблему из области casual
> inference: чтобы получить точную оценку качества алгоритмов, необходимо знать ответы клиента по
> каждой из рук. То есть одновременно предложить клиенту сразу все руки… а это невозможно.**»

**Чем они её решают — дословно:**

> «В самом базовом варианте мы **моделировали тестовую систему распределениями Бернулли**: смотрели
> конверсии по рукам на истории и передавали эти параметры в распределения. Для каждого клиента в
> симуляции мы создавали ряд сценариев развития действий…»

Методологические детали, полезные как эталон оформления (дословно): кластеризация K-Means на
**15 кластерах**, `squaredEuclidean`, **silhouette score 0,65**; слияние кластеров размером менее
**10 %** выборки; SMOTENC для баланса классов — «целевое соотношение минорного класса к мажорному
— **5 %** (изначальное — около **1 %**), количество ближайших соседей — **3**»; «Случайный лес»
(`n_estimators: 1000`, `max_depth: 30`, `class_weight: "balanced"`), обучающая доля **80 %**;
перебранные политики — ε-greedy, Softmax, UCB, Thompson Sampling; библиотека `space_bandits`
на моделях из «Deep Bayesian Bandits Showdown».

🔴 **Как это меняет отрицательный результат §П4.4 D — уточняет, а не отменяет.** (1) Пункт «ВТБ:
публикация с методологией не найдена» **закрыт**: публикация есть, с кодом, параметрами и числами.
(2) Пункт «off-policy оценка (IPS/DR) в продакшене российского банка не найдена» **остаётся верным
и становится ТОЧНЕЕ**: российский банк дошёл ровно до той развилки, где нужен IPS/DR, назвал её
своими словами как задачу causal inference — и выбрал **симулятор на распределениях Бернулли**,
а не оценку по логам через IPS/DR. Плюс сами авторы признают, что **в полноценный пилот это не
запускалось**. То есть для темы 37 корректная формулировка теперь такая: «в открытых российских
источниках off-policy оценка по логам не описана; ближайший найденный случай — ВТБ 2025, где та же
задача решается симуляцией, и результат получен в тестовой среде, а не в проде».

## Г30.1-16. Дополнительно по российскому контуру: два кейса причинной оценки с числами

Оба найдены Exa по описанию задачи, оба ранее в файле не значились.

1) **Альфа-Банк + GlowByte, «Оценка эффекта персонализации баннеров… Kolmogorov ABacus»**, Хабр,
`https://habr.com/ru/companies/glowbyte/articles/762164/`, 20.09.2023 (через Exa, 16.09.2026).
🔴 **Кейс, в котором неверная сборка групп дала ЛОЖНЫЙ отрицательный вывод — дословно:**
> «когда был запущен пилот для оценки экономического эффекта от внедрения этой модели, оказалось,
> что **по большинству баннеров он отрицателен (!)**, а по остальным эффекта почти нет.»
> …
> «Именно такие симптомы мы обнаружили при анализе групп, принимавших участие в экспериментах
> Альфа-Банка… **до применения постстратификации эффект моделей выглядел статистически незначимым,
> но постстратификация восстановила справедливость: модели доказали свою эффективность по всем
> рекомендуемым ими баннерам.**»
Метод формирования групп (дословно): «стратификация через **HDBScan**-кластеризацию… он устойчиво
демонстрировал **уровень ошибки первого рода ниже допустимого предела в 5 %**» на А/А-тестах.
**Для нас:** это документированный российский пример того, что **вывод «эффекта нет» может быть
артефактом несопоставимости групп**. Прямо относится к нашему разделу про риски интерпретации
пилота: отрицательный результат обязан сопровождаться проверкой сопоставимости (А/А + доли страт).

2) **Альфа-Банк, AUF (Automatic Uplift Framework), open source**, Хабр
`https://habr.com/ru/companies/alfa/articles/1024090/` (22.04.2026) и первая часть
`https://habr.com/ru/companies/alfa/articles/895002/` (28.03.2025) — через Exa.
Дословно: «поверх AUF было настроено **автопереобучение в прод инфраструктуре Банка**… Если метрика
ухудшилась, **Jenkins сам запустит пайплайн переобучения**… **Ускорение — в 5–10 раз!** Вместо двух
недель работа займет всего один рабочий день». Условие корректности (дословно): «**Гомогенность
выборки** — это свойство, которое необходимо для построения uplift-модели. Оно означает, что
**назначение воздействия было рандомным**… Это позволит при моделировании оценить чистый эффект».
**Для нас:** подтверждает вывод §A о том, что uplift-моделирование в РФ — промышленная практика,
и добавляет прямую формулировку требования рандомизации как условия применимости.

## ИТОГ Г30.1 — causal_effect_measurement

| Пункт | Был статус | Стал | Чем взят |
|---|---|---|---|
| Polonetsky, Tene & Jerome 2015, полный текст | НЕ ДОБЫТ — отказ по 5 каналам (curl 403, r.jina.ai пустышка 228 б, Wayback 404, archive API 429, SSRN 403) | 🔴 **ДОБЫТ** (60 228 байт; §I–§IV.B) | `mcp__exa__web_fetch_exa` пробил антибот Colorado Law |
| Meyer 2015 «Two Cheers» | не искался (бюджет) | **добыт** (титул, оглавление со страницами, введение) | Exa fetch того же репозитория, article=1194 |
| Freedman 1987, полный текст | НЕ ДОБЫТ; «формулировка про остановку не получена» | **частично добыт**: три дословные цитаты тела статьи, одна со с. 144, включая «вторую часть» определения | Exa → открытая BMJ/PMC-статья + Springer Encyclopedia |
| Кодекс этики ИИ 2021, полный текст | НЕ ДОБЫТО (не качал) | 🔴 **ДОБЫТ ПОЛНОСТЬЮ** (51 831 байт текста) | `curl -sk --http1.1` → ethics.a-ai.ru, HTTP 200, 3 095 695 байт PDF |
| ВТБ: публикация с методологией | НЕ НАЙДЕНА | 🔴 **НАЙДЕНА** (Хабр, 05.06.2025, с параметрами и числами) | Exa-поиск по описанию задачи, а не по имени компании |
| Off-policy (IPS/DR) в проде российского банка | не найдено ни одного описания | **отрицательный результат подтверждён и уточнён**: ВТБ доходит до той же развилки и решает её симулятором, пилота не было | Exa |
| Доля трафика на эксперименты в российском банке | чисел нет | **чисел по-прежнему нет**; но добыт смежный числовой ориентир (конверсия 0,22 %, 220 откликов из 100 000) | Exa |
| Видео доклада Альфа-Банка (RUTUBE) | не просматривалось (нет транскрипции) | **не изменилось** — канала транскрипции по-прежнему нет | — |
| X5 Tech (Хабр 403) | не добыто, вне приоритета | **не переоткрывалось** (ритейл, вне приоритета) | — |

**Главное число по файлу `causal_effect_measurement`: из 9 открытых пунктов Exa перевела в
«добыто» или «частично добыто» — 6** (Polonetsky, Meyer, Freedman-частично, Кодекс этики ИИ,
ВТБ-публикация, плюс два незапланированных кейса Альфа-Банка сверх списка). Не сдвинулись 3:
доля трафика (чисел не публикуют), RUTUBE-доклад (нет транскрипции), X5 (вне приоритета).
🔴 Ключевой замер батча: **Exa взяла страницу, на которой `curl` и `r.jina.ai` давали 403** —
именно этот класс отказов и делал прежние «не добыто» непроверенными.

---

## ДОБОР Г31.2 — прокси (16.09.2026)

**Каналы на начало работы (16.09.2026, замер, а не предположение):**
`r.jina.ai` **без браузерного UA** → `https://r.jina.ai/https://www.monarchmoney.com/pricing` — **HTTP 200, 5 509 байт** полного текста;
он же **с браузерным UA** (Chrome/127) по тому же адресу — **HTTP 403, 5 743 байта**, тело `<title>Just a moment...` (Cloudflare самого прокси).
🔴 Расхождение подтверждено прямым парным замером: **ошибка вызова прокси с UA реальна**, прокси жив.
`mcp__exa__web_search_exa` / `web_fetch_exa` — 🟢 работают. `curl` с UA — работает.

### Г31.2-1. 🔴 X5 Tech, «От A/B-тестирования к Causal Inference в офлайн ритейле» — ПОЛНЫЙ ТЕКСТ ДОБЫТ (был «Хабр 403, вне приоритета»)

**Прежний статус в этом файле** (строки ~3425, ~3548, ~3996): `habr.com/ru/companies/X5Tech/articles/768008/`
→ **403, 67 081 байт** (антибот Хабра именно на этой статье); в Г30.1 помечено «не переоткрывалось
(ритейл, вне приоритета)».

**Каналы 16.09.2026:**

| Канал | Код | Размер | Итог |
|---|---|---|---|
| `r.jina.ai` **без UA** | HTTP 200 у прокси | 2 684 б | тело: `Warning: Target URL returned error 403: Forbidden` — прокси антибот Хабра не пробил |
| `mcp__exa__web_fetch_exa` | 200 | ~28 000 знаков выдано (статья на 31 мин чтения) | 🟢 **полный текст, включая код и формулы** |

🔴 **Вывод по каналу: это НЕ была наша ошибка вызова прокси.** Прокси без UA даёт тот же 403
от Хабра. Взяла статью **Exa** — тот же класс, что Polonetsky в Г30.1.

**Реквизиты:** `habr.com/ru/companies/X5Tech/articles/768008/`, автор `nikita_volkov`,
опубликовано **19.10.2023 09:55**, блог компании X5 Tech, команда Ad-Hoc аналитики,
33 K просмотров, метка «Туториал».

#### Первичный материал, дословно (ключевые фрагменты)

> «Приветствуем всех читателей! Сегодня команда Ad-Hoc аналитики X5 Tech приоткроет дверь
> в увлекательный мир A/B-тестирования Causal Inference. С момента написания предыдущей статьи
> прошло уже 4 года… мы пришли не только к другой модели для оценки эффекта, но и изменили
> методологию в целом.»

> «В X5 эксперименты обладают глубинным смыслом – они являются частью инвестиционного процесса.
> Это означает, что продукты обязаны подтверждать эффективность своих идей для получения
> дальнейшего финансирования.»

**Замер смещения при неслучайной пилотной группе (дословно, с кодом):**

> «Есть заказчик Вася… Но в процессе деловой встречи выясняется, что Вася уже провёл
> эксперимент… в Екатеринбурге… на 20% магазинов с самой высокой выручкой за предыдущий год.»
> «При проведении 10 000 симуляций выясняется, что статистически значимая разница средних
> проявляется в **~12% случаев**, хотя мы установили уровень значимости в **5%**. Таким образом,
> ошибка I рода не контролируется на заданном уровне значимости. При более экстремальных
> отклонениях тестовой выборки от генеральной совокупности эта ошибка легко может стремиться
> к **100%**.»
> Воспроизводимый код: `MEAN = 200000.0, STD = 30000.0, CUTOFF = 150000.0, N_SAMPLES = 100,
> N_SIM = 10000, ALPHA = 0.05, np.random.seed(1)` → вывод «Эмпирическая ошибка 1 рода: **0.1187**».

**Где кончается A/B и начинается causal inference (дословно):**

> «Именно здесь заканчивается A/B-тестирование и начинается Causal Inference, основной задачей
> которого является выявление причинно-следственных связей.»
> Перечень ситуаций, когда рандомизация физически невозможна: пилот меняет распределительный
> центр; влияет на промо-продажи, зависящие от географии; реализуем только в рамках целого
> региона из-за ограничений контрагента; затрагивает наружную рекламу (раскатка только
> на уровне города).

**Пайплайн модели (дословно, пять шагов):**

> «Извлекаем признаки (фичи) из данных. Понижаем размерность пространства фичей и отбираем среди
> них признаки, коррелирующие с целевой метрикой на периоде пилота. На основе полученных ранее
> фичей строим модель оценки вероятности попадания магазина в ПГ или КГ – propensity score…
> Производим процедуру тримминга propensity score… Производим оценку эффекта с помощью модели
> на основе линейной регрессии, используя propensity score для перевзвешивания объектов,
> а также все полученные ранее фичи для повышения чувствительности.»

> «Построенная таким образом итоговая модель оценки эффекта является композицией нескольких
> моделей и обладает свойством **Doubly Robust**: оценка эффекта является корректной, если
> **хотя бы одна** из составляющих её моделей корректно специфицирована.»

**Смещение и чувствительность решаются ОДНОВРЕМЕННО (дословно) — прямо против «просто возьмите CUPED»:**

> «В различных источниках обычно встречаются только методы повышения чувствительности, такие
> как CUPED. Но, как мы помним, в большинстве наших задач ПГ может иметь сильное смещение
> относительно КГ. Отметим, что задачи повышения чувствительности и уменьшения смещения должны
> решаться одновременно, так как иначе мы можем повышать чувствительность, не уменьшая смещение.
> В этом случае доверительные интервалы будут строиться с центром в **ложно оценённом эффекте**,
> что чревато высокой ошибкой I рода.»

**Калибровка propensity score (дословно) — против «возьмём градиентный бустинг»:**

> «При оценке propensity score может возникнуть соблазн использовать различные сложные техники
> машинного обучения. Однако, чаще всего это **неверный путь**, поскольку в данном случае мы
> стремимся не к высокой точности прогнозирования, а к **несмещённым оценкам вероятностей**…
> очень уверенная модель, верно классифицирующая выборку, и предсказывающая вероятности
> в районе нуля или единицы, **не позволяет** использовать такие оценки вероятностей для
> сопоставления выборок… мы используем калибровку (Calibrated Classifier CV)… Кроме того,
> **не стоит использовать сильную регуляризацию** в модели логистической регрессии при оценке
> propensity score.»

**Тримминг и Common Overlap (дословно):**

> «Для получения несмещённых оценок эффекта необходимо выполнение условия на propensity score,
> которое носит название **Common Overlap (Common Support, Positivity Check)**… Отметим, что для
> достижения сопоставимости групп важно применять **одинаковые правила очистки к обеим группам**.
> Например, можно посчитать квантили только по пилотной группе, но удалить по ним выбросы
> необходимо из обеих групп сразу. В зависимости от размера групп можно производить несколько
> раундов тримминга с разными правилами, **каждый раз заново оценивая propensity score**.»

**Веса PSW (дословно):** вес объекта в ПГ ∝ `1/ê`, в КГ ∝ `1/(1−ê)`; числовой пример авторов —
ПГ: 8 магазинов «единороги» (ê = 4/5, вес 5/4, сумма 10) и 1 «бегемоты» (ê = 1/6, вес 6, сумма 6);
КГ: 2 «единороги» (вес 5, сумма 10) и 5 «бегемоты» (вес 6/5, сумма 6) → «после взвешивания мы
получили сопоставимые ПГ и КГ». Из трёх приёмов (PSM / PSB / PSW) авторы выбрали PSW: «по
результатам наших экспериментов… **наилучшие результаты показывает данный подход со взвешиванием**».

**Оценка эффекта:** взвешенная линейная регрессия, все признаки, константа и таргет домножаются
на нормированный вес, **кроме** бинарного признака тритмента — «поскольку мы хотим, чтобы разница
в предсказании при изменении с 0 на 1 … была бы равна эффекту»; далее устойчивые
к гетероскедастичности стандартные ошибки и доверительный интервал по Стьюденту; «результат
называем статистически значимым, если этот интервал не содержит 0».

**Связь с diff-in-diff (дословно):** «В этом процессе взятия разностей можно заметить некоторый
аналог известного подхода **diff-in-diff**… мы из приращения целевой метрики между пилотным
и препилотным периодами в ПГ вычитаем аналогичное приращение в КГ».

**Отбор признаков:** PCA → оставляют только фичи со статистически значимой корреляцией с таргетом
и/или тритментом, топ-N по модулю корреляции. «Лишние фичи, которые не оказывают никакого влияния…
наоборот, могут **повысить дисперсию** оценки эффекта». Инструментальные признаки в модель
**не берутся** («она предназначена для работы только с ковариатами и с конфаундерами»).

#### Выжимка — что это даёт нашей теме

1. 🔴 **Это второй российский первоисточник (после ВТБ, Г30.1-15), где промышленная команда
   описывает ровно нашу развилку** — «рандомизировать нельзя, оценивать надо» — и решает её
   не симулятором, а **doubly robust с propensity score weighting**. Для §Д1/Д2 файла это меняет
   фактуру: раньше российский контур выглядел как «ВТБ дошёл до развилки и остановился»; теперь
   есть описанный до формул промышленный пайплайн, пусть и в офлайн-ритейле, а не в финпродукте.
2. 🟢 **Числовой якорь против наивного сравнения групп:** при мягком отборе (выручка выше 150 тыс.
   при среднем 200 и σ 30) ошибка I рода **12 % вместо 5 %**, при жёстком — «стремится к 100 %».
   Это готовая, воспроизводимая (seed=1) иллюстрация того, почему «посмотрим на тех, кто принял
   совет, против тех, кто не принял» в нашем продукте даст ложный эффект.
3. 🔴 **Прямое предупреждение против пути «CUPED и достаточно»**: снижение дисперсии без снижения
   смещения строит доверительный интервал вокруг ложного эффекта. Если мы когда-нибудь будем
   мерить эффект советов FINPILOT на самоотобранной группе принявших — смещение первично.
4. 🟢 **Практическая деталь калибровки**, которую легко сделать неправильно: propensity score
   нужен несмещённый, не точный; сильная регуляризация и «уверенная» ML-модель ломают
   сопоставление. Плюс: тримминг повторять с переоценкой propensity score.
5. ⚪ **Границы переноса:** единица наблюдения — магазин, не человек; горизонты — недели пилота;
   офлайн-ритейл. Прямого переноса на «пользователь принял совет» нет, переносится **метод**,
   а не числа.

**Меняет ли канон, прогноз, новизну, юрблок:** нет. Это материал §Д1–Д2 (практика причинной
оценки) и аргумент для будущего дизайна замера эффективности продукта. Канон модели v3.0.0,
формулировку новизны и код не трогаю.

### Г31.2-2. Пункты этого файла, перепроверенные и НЕ сдвинувшиеся

| Пункт | Каналы 16.09.2026 | Итог |
|---|---|---|
| Доля трафика на эксперименты в российском банке | не переоткрывался | класс «числа не публикуют», прокси этого не лечит |
| Видео доклада Альфа-Банка (RUTUBE) | не переоткрывался | нет канала транскрипции; не проблема антибота |
| Freedman 1987, полный текст | не переоткрывался | закрыт пейволлом; прокси пейволл **не** обходит (подтверждено на Wiley/SSRN ниже) |

## ИТОГ Г31.2 — causal_effect_measurement

- Закрыт **1 пункт** (X5 Tech, полный текст) — и он был **не** нашей ошибкой вызова прокси:
  прокси без UA получает тот же 403 от Хабра, взяла Exa.
- 🔴 **Главный канальный результат файла:** парный замер подтвердил, что `r.jina.ai`
  с браузерным UA отдаёт капчу **своего** Cloudflare (403), а без UA — 200. Ошибка вызова реальна,
  но на этом файле ни одного отказа она не объясняет.
...
попыток**; `/paper/search/bulk`, `/paper/DOI:<doi>`, `/paper/DOI:<doi>/citations` — **200**.

### Сверка списка «не добыто» перед работой (правило «последнее упоминание»)

Прошёл §И4 и §4.2 и отделил то, где S2 действительно был непройденным каналом, от того,
где он уже применялся или неприменим:

| Пункт | Решение | Основание |
|---|---|---|
| **Freedman 1987, clinical equipoise** | ⛔ **в добор не берётся** | **S2 уже применялся** и зафиксирован в §2 этого файла: «Semantic Scholar Graph API (`paper/DOI:…`) — HTTP 200, `openAccessPdf.status = "CLOSED"`». Канал пройден, результат отрицательный; дословный абстракт добыт через EuropePMC |
| **Radcliffe 2007 (Qini)** | ⛔ не берётся | последнее упоминание — «закрыт как **не требуется**»: определение Qini взято из рецензируемого источника, цитирующего Radcliffe напрямую |
| **Abadie 2021, JEL 59(2):391–425** | 🟢 **берётся** | S2 по нему не пробовался ни разу; пробовались `curl` MIT, `curl` aeaweb, `r.jina.ai` (404 и 1 710 б только аннотация), NBER |
| FCA Consumer Duty, CFPB, DSA ст. 40, EU AI Act, корпоративные IRB | ⛔ не берутся | правовые и корпоративные документы, S2 их не индексирует; заход был бы имитацией |
| Доля exploration-трафика в проде; формулировки из сниппетов (§1.3) | ⛔ не берутся | не научные публикации с DOI |
| X5 Tech | ⛔ не берётся | ритейл, вне приоритета, последнее упоминание — «не переоткрывалось» осознанно |

### Г31.4-E1. 🟢 Abadie A. (2021) — ДОБЫТ ПОЛНЫМ ТЕКСТОМ, журнальная вёрстка JEL

Единственный пункт этого файла, по которому S2 был непройденным каналом, — и он закрылся.

| Шаг | Адрес | Код / размер |
|---|---|---|
| Unpaywall `v2/10.1257/jel.20191450` | api.unpaywall.org | **200, 2 169 б**; 🔴 **`is_oa: true`, `oa_status: "green"`**, единственная локация — `hdl.handle.net/1721.1/144417` (MIT), `version: submittedVersion` |
| **Semantic Scholar `paper/DOI:…`** | api.semanticscholar.org | **200, 1 757 б**; `openAccessPdf.status: "GREEN"`, 🔴 **`license: "CCBYNC"`**, `citationCount: 1236`; **абстракт и `tldr` отданы полностью** |
| `hdl.handle.net/1721.1/144417` прямой | MIT | **405 Method Not Allowed**, 2 115 б |
| тот же адрес через `r.jina.ai` | — | **200, 845 б** — «Human Verification… Target URL returned error 405» |
| DSpace 6 REST `/rest/handle/…` | dspace.mit.edu | **404** (443 468 б — SPA-заглушка) |
| 🟢 **DSpace 7 API, поиск** `/server/api/discover/search/objects?query=…` | dspace.mit.edu | **200, 294 940 б** → uuid `667a4480-667d-4844-889a-050157f54f4c`, handle `1721.1/144417` |
| 🟢 `/server/api/core/items/<uuid>/bundles` | — | **200, 4 283 б** → bundle `ORIGINAL` |
| 🟢 `/server/api/core/bundles/<uuid>/bitstreams` | — | **200** → `jel.20191450.pdf`, 882 642 б |
| 🟢 **`/server/api/core/bitstreams/4615246c-50b2-4231-b0bd-74ea96847d84/content`** | — | **HTTP 200, 882 642 б, `application/pdf`**, sha256 `f9b2748cd28a5021734d5a566f11d24b58c87b47ea0143d62bf43504a13d5b0e` |

🔴 **Приём, который стоит запомнить отдельно — он общий, а не про эту статью.** Handle MIT
отдаёт **405** и капчу прокси, но **DSpace 7 REST API открыт без ключа**, и по нему файл
достаётся в три шага: поиск → `bundles` → `bitstreams` → `content`. Прежние заходы
упирались в handle и в `aeaweb.org` и записывали пункт недобытым. Это ровно класс Г31.5
(«отказ ОДНОГО адреса записан как свойство источника»), и он даёт готовый рецепт для любого
репозитория на DSpace 7.

**И ещё одна поправка к записи.** Unpaywall помечает копию как `submittedVersion`, но
добытый файл — **журнальная вёрстка**: первая строка «Journal of Economic Literature 2021,
59(2), 391–425», колонтитул «Abadie: Using Synthetic Controls 413», лицензия CC BY-NC по S2.
То есть метка версии у Unpaywall в этом случае **неверна**, и полагаться на неё вслепую нельзя.

**Абстракт ДОСЛОВНО (с. 391):**

> «Probably because of their interpretability and transparent nature, synthetic controls
> have become widely applied in empirical research in economics and the social sciences.
> This article aims to provide practical guidance to researchers employing synthetic control
> methods. <…> The main sections discuss the advantages of the synthetic control framework
> as a research design, and **describe the settings where synthetic controls provide reliable
> estimates and those where they may fail.** <…> (JEL B41, C32, C54, E23, F15, O47)»

**Структура требований — теперь из первоисточника, а не из пересказа Ben-Michael et al.**
Раздел 5 «Contextual Requirements»: *Availability of a Comparison Group* (с. 411),
*No Anticipation* (с. 411), *Convex Hull Condition* (с. 412). Раздел 6 «Data Requirements»
(с. 413): *Aggregate Data on Predictors and Outcomes*, *Sufficient Pre-intervention и
Post-intervention periods*.

**Дословно, о необходимости длинного дотретментного окна и о его границе (с. 413):**

> «the synthetic control method **requires the availability of data on outcomes and predictors
> of the outcome** for the unit or units exposed to the intervention of interest and a set of
> comparison units… for **the affected unit and the donor pool for a large pre-intervention
> window**. **A caveat to the preference for a large number of pre-intervention periods is given
> by the possibility of structural breaks.** <…> Even if the model is a good representation of
> the distribution of the data at a relatively short time scale, its accuracy may suffer once we
> allow the number of periods to be large enough.»

И дословно о малом окне (с. 413): «With a small number of pre-intervention periods, close or
even perfect fit of the predictor values for the treated unit may be **spurious**» — то есть
хороший подгон на коротком ряду сам по себе ничего не доказывает.

🔴 **Что это меняет у нас: НИЧЕГО по выводу и многое по прочности вывода.** Участок 2.4 был
закрыт отрицательным заключением (синтетический контроль нам не подходит), и заключение
**подтверждается первоисточником, а не отменяется им**. Но теперь оно опирается на дословные
требования автора метода, а не на цитаты из третьей работы: нам нужны **агрегированные ряды
по единице воздействия и донорскому пулу за длинное дотретментное окно**, причём
структурно стабильное, — а у персонального финансового советника единица наблюдения
это отдельное домохозяйство с коротким и структурно нестабильным рядом. Пометка §И4 п. 1
«влияние на результат: нулевое» **остаётся верной**; меняется только то, что аргумент
теперь цитируемый. **Канон, новизна и код не затрагиваются.**

## ИТОГ Г31.4

По этому файлу: **1 пункт закрыт, 6 сознательно не брались с названным основанием.**
- 🟢 **Abadie 2021 (JEL 59(2):391–425) ДОБЫТ ПОЛНЫМ ТЕКСТОМ**, журнальная вёрстка, CC BY-NC.
  Требования метода (Contextual Requirements, Data Requirements) теперь цитируются
  из первоисточника, а не из пересказа Ben-Michael et al. **Вывод участка 2.4
  не меняется — он подтверждается.**
- 🔴 **Приём общего назначения, добытый попутно:** handle MIT отдаёт **405** и капчу прокси,
  но **DSpace 7 REST API открыт без ключа**: `search/objects` → `items/<uuid>/bundles` →
  `bundles/<uuid>/bitstreams` → `bitstreams/<uuid>/content`. Применимо к любому
  репозиторию на DSpace 7.
- 🔴 **И предупреждение:** Unpaywall пометил эту копию `submittedVersion`, а файл оказался
  **журнальной вёрсткой**. Метке версии у Unpaywall доверять вслепую нельзя.
- Не брались: Freedman 1987 (**S2 по нему уже применялся** и зафиксирован в §2 — канал
  пройден), Radcliffe 2007 (закрыт как «не требуется»), FCA/CFPB/DSA ст. 40/EU AI Act/IRB
  (правовые документы, S2 не индексирует), доля exploration-трафика и формулировки
  из сниппетов (не публикации с DOI), X5 Tech (вне приоритета).
**Вклад Semantic Scholar:** подтвердил `GREEN` и отдал абстракт, но тот же адрес дал
Unpaywall, а файл достал DSpace API. **Не решающий.**
Задолженности нет. 🔴 Канон, новизна, код — не затронуты.
