...
# ДОБОР Г7.3 (11.09.2026) — Starling round-ups (первоисточник) и KakaoBank (IR добыт)

## Г7.3.1 — Starling Round Ups: 🟢 ДОБЫТО с официального сайта банка

🔴 **Почему `help.starlingbank.com` не открывался — причина установлена точно.**
`https://help.starlingbank.com/personal/topics/using-the-app/what-are-starling-spaces/`
→ `curl -sk`+браузерный UA **HTTP 200, но всего 2 601 байт**, тело целиком:
«Starling Customer Service. **You need to enable JavaScript to run this app.**» То же самое
получил `WebFetch` («страница содержит только заголовок»). Справочный центр Starling —
это **SPA без серверного рендера**, поэтому ни один текстовый канал его не возьмёт;
это не блокировка и не антибот. **Рабочая замена найдена: продуктовая страница на основном
домене**, `https://www.starlingbank.com/features/round-ups/` → `curl -sk`+UA
**HTTP 200, 108 709 байт**, текст отрендерен полностью.

**Механика Round Ups — дословно с `starlingbank.com/features/round-ups/` (снято 11.09.2026):**

> «Every time you buy something on your Starling **personal, joint or business account**, we'll
> **round up your spending to the nearest pound** and add the spare change into a **Regular Space**
> of your choice. Spend **£3.40** on a coffee? We'll round it up to **£4.00** and add **60p**
> to your Space.»

> «To reach your goals even faster, try **multiplying** the spare change you save – increase to
> **x2, x5 or x10** in your app. You can also top up your Space with **one-off or scheduled
> payments**.»

> «Once you're ready to spend, simply transfer money from your Space to your main account …
> or **set up a virtual card to spend straight from the Space**.»

**Включение — не по умолчанию, а вручную, дословно:** «Head to 'Spaces' in your Starling app.
Create a new Regular Space or tap on an existing one. Click '**Manage Space**,' then
'**Round Ups**' and switch the **toggle on**.»

🔴 **Четыре факта, важных нам, и все — против расхожего описания round-ups:**
1. **Округление только до целого фунта**, шага и порога нет; множитель ×2/×5/×10 — единственный
   способ увеличить откладываемое. То есть сумма накопления — **функция числа транзакций**,
   а не дохода и не цели.
2. **Функция выключена по умолчанию** и включается на конкретное «Space» — то есть это
   **opt-in**, а не автоматическое поведение банка. Ссылки на round-ups как на пример
   «автоматизации сбережений по умолчанию» (default nudge) в этой части **неверны**:
   дефолт здесь — «выключено».
3. **Деньги не блокируются**: из Space можно как перевести обратно, так и **тратить прямо
   с виртуальной карты**. Барьера к изъятию, на котором обычно строят поведенческий эффект,
   нет вовсе.
4. Round Ups доступны и на **бизнес-счёте**, не только на личном — это редкость.
5. Отмечено обзором рынка (🟡 вторичный источник, `moneytothemasses.com`): множитель у
   Starling бесплатен, «something that some rival banks charge a subscription fee for» —
   то есть у конкурентов ровно эта механика бывает платной функцией подписки.

**Чем это полезно нам.** Round-ups — механика **накопления мелочи**, ортогональная нашей
задаче распределения свободного потока: она не спрашивает про цели, не сравнивает
альтернативы и не считает выгоду. Как конкурент нашему движку она не выступает; как
**механика вовлечения** — да, и её ключевая настройка (opt-in, обратимо, множитель ×2/×5/×10)
задаёт планку простоты, с которой пользователь сравнивает всё остальное.

## Г7.3.2 — 🔴 KakaoBank: первоисточник IR ДОБЫТ, числа платформенного дохода перестали быть непроверенными

**Как добыто — приём, которого раньше не было.** Прямые адреса дают 404: `www.kakaobank.com/ir`,
`/ir/archive`, `/en/ir`, `/ir/irData` → все **HTTP 404, 24 754 байта**; `ir.kakaobank.com` →
**HTTP 000** (домена нет). Рабочая страница — `https://www.kakaobank.com/view/report/archive/earnings`
(**HTTP 200, 47 048 байт**), но список файлов на ней **пустой**, потому что грузится
скриптом. Дальше:
1. из HTML вытащен `src="/view/js/KO/views/report/archive/earnings.js"` → **HTTP 200, 5 256 байт**,
   внутри строка `url : "/view/api/report/archive/performance/list"`;
2. из `/view/js/KO/pagination.js` (**HTTP 200, 12 361 байт**) снят формат запроса:
   POST, JSON, поля `page` (**с нуля**) и `pageSize`;
3. `POST https://www.kakaobank.com/view/api/report/archive/performance/list`
   с телом `{"page":0,"pageSize":10,"searchKeyword":""}` → **HTTP 200, 6 290 байт JSON**,
   `resultCode:"0000"` — полный реестр квартальных презентаций с **прямыми ссылками на PDF
   (корейский и английский) и на XLSX-фактшиты** в Azure Front Door.
   (Первая попытка с телом `{"page":1,"size":20}` дала **HTTP 500** — поля названы иначе.)
4. Английская презентация за II квартал 2026:
   `https://fde-kkb-prod-krc-stkkbprodkrcblob-g4h6eybzded7e9ha.a01.azurefd.net/prod/20260804%2F9fabc16727934c75a1dd77d449c5a678.pdf`
   → `curl -sk`+UA **HTTP 200, 1 155 196 байт**; `pdftotext -layout` → 55 698 символов.

🟢 **Приём стоит записать отдельно:** когда IR-архив банка отдаёт пустой список, ответ —
не «не добыто», а найти JS-обработчик страницы и вызвать его API напрямую. Здесь это
заняло три запроса и закрыло единственный банк из десяти, оставшийся незакрытым.

### Числа из первоисточника (KakaoBank, 2Q26 IR presentation, млрд вон)

**Отчёт о прибылях (Appendix: Income Statement), 2Q26 против 2Q25:**

| Строка | 2Q26 | 2Q25 | YoY | 1Q26 | QoQ |
|---|---|---|---|---|---|
| Operating Revenue | **829,0** | 778,0 | +6,5 % | 819,3 | +1,2 % |
| Interest Revenue | 690,4 | 604,1 | +14,3 % | 659,7 | +4,7 % |
| Fee & Commission Revenue | 88,7 | 75,9 | +17,0 % | 80,8 | +9,7 % |
| — Fee Revenue | 63,4 | 52,1 | +21,9 % | 57,4 | +10,6 % |
| — **Platform Revenue** | **25,3** | 23,8 | **+6,2 %** | 23,4 | +7,8 % |
| Fee & Platform Expense | 52,5 | 47,1 | +11,5 % | 50,3 | +4,4 % |
| Operating Profit | 194,0 | 170,1 | +14,0 % | 157,6 | +23,1 % |
| Net Profit | **140,8** | 126,3 | +11,5 % | 187,3 | −24,9 % |

🔴 **Ключевое наблюдение, которое меняет прежнюю запись файла.** Платформенный доход
KakaoBank — **25,3 млрд вон из 829,0 млрд операционного дохода, то есть примерно 3 %**.
Процентный доход — **83 %**. Более того, **расходы по строке «Fee & Platform Expense»
(52,5) вдвое превышают весь платформенный доход (25,3)**. Тезис «KakaoBank зарабатывает
на платформе» в источниках этого файла держался на непроверенных числах; по первоисточнику
KakaoBank — **обычный процентный банк с платформенной витриной**, и витрина эта убыточна
в прямом сопоставлении строк отчёта.

**Состав платформенного дохода за 2Q26 (круговая диаграмма «Key Components»):**
**Loan comparison 34 % · Ads 31 % · Credit card issuance platform, mini и прочее 25 % ·
Brokerage account referral 10 %**. То есть три четверти платформенных денег — это
**реферальные выплаты партнёров за приведённого заёмщика и реклама**, ровно та модель,
что у Credit Karma (см. добор Г7.2 в `competitor_marketing_positioning`).

**Прочие числа с тех же слайдов:**
- **Клиентская база — 27,63 млн** (+0,93 млн с начала года); **MAU 20,3 млн**, **WAU 15,0 млн**
  (среднеквартальные, Nielsen Media Korea Digital Data + данные компании).
  Проникновение по возрастам, 2Q26: 30-летние **88 %**, 40-летние **80 %**, 20-летние **83 %**,
  50-летние **63 %**, 60+ **22 %**, до 19 лет **31 %**.
- **Loan comparison execution volume: 1 560 млрд вон** во 2Q26, **+12 % г/г и +17 % кв/кв**;
  партнёров по необеспеченным кредитам и ипотеке — **72** в 1H26 против 66 в 1H25 (**+6**);
  запуск автокредитования — **сентябрь 2026**.
- 🔴 **Рекламная выручка +81 % г/г**, трафик **+38 % г/г**; доля рекламы внутри платформенной
  выручки выросла с **19 % (2025) до 31 % (2Q26)**. Это самая быстрорастущая часть платформы.
- Оборот по дебетовым картам **+5 % г/г**; запуски августа 2026 — **Payment Home**
  («consolidating transaction history, rewards, and performance metrics») и **PLCC**
  с выгодой до **50 тыс. вон в месяц** на «simple payments, AI subscriptions, and membership
  services».

### 🟡 Что по KakaoBank всё ещё не добыто

**Блог про теги и заметки к транзакциям** — в этом доборе не искался (бюджет ушёл на IR).
Функциональность PFM в презентации 2Q26 описана только через «Payment Home» (сведение
истории операций, вознаграждений и показателей) и «Investment Tab»; слов MyData, тегов,
заметок или категоризации в английской презентации **нет ни разу** (проверено `grep` по
всем 55 698 символам: «mydata», «tag», «memo», «record» — совпадений по смыслу нет).
То есть прежняя запись файла «PFM есть (агрегация MyData + анализ расходов), детали не
добыты» **первоисточником IR не подтверждается**: в материалах для инвесторов KakaoBank
свой PFM как продукт не позиционирует вовсе.

🟢 **Реестр остальных PDF получен целиком** (квартальные презентации с 2021 по 2Q26,
корейская и английская версии + XLSX-фактшиты), адреса лежат в ответе API — при следующей
теме по KakaoBank добывать заново не нужно, достаточно повторить POST.
