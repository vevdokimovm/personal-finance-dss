...
**Каналы (замер 17.09.2026):** Unpaywall **200** · Crossref **200** · S2 `/paper/DOI:` **200** · `r.jina.ai` без UA **200** · Exa fetch — `CRAWL_LIVECRAWL_TIMEOUT` на `link.springer.com`.

### Г31.5-R1. Sharaf et al. 2022 — ОТКЛОНЁН как кандидат класса: пейволл подтверждён пятью адресами

Последнее упоминание — стр. 931: `WebFetch` 303 → `idp.springer.com`; `r.jina.ai` 200, только литература; S2 без PDF. Это уже не «один адрес», но заход повторён для полноты:
- Unpaywall `10.1007/s11042-022-12564-1` — **200**, `is_oa: False`, `closed`.
- Crossref — **200, 31 658 б**: `abstract: None` (издатель абстракт не депонирует); реквизиты **MTA 81(12):16761–16781**; лицензии только `springer.com/tdm`.
- S2 `/paper/DOI:` — **200, 1 148 б**, абстракт изъят, `citationCount: 36`; 🟢 **`tldr` ДОСЛОВНО** (машинная аннотация): «This paper presents a review of the recommendation system, its types, and its applications, and concentrated on the finance recommendationSystem, its operation, and the different finance sectors.»
- `r.jina.ai` без UA по `link.springer.com/article/10.1007/s11042-022-12564-1` — **200, 34 340 б**: снова только список литературы (антибот пройден, пейволл — нет).
- Exa `web_fetch_exa` по той же странице — `CRAWL_LIVECRAWL_TIMEOUT`.

**Итог:** 🔴 не класс Г31.5 — источник закрыт по существу. Новое — только точные страницы тома и `tldr`, который подтверждает, что обзор описательный (типы RS и секторы финансов), а не методологический; потеря для вопросов темы мала.

## ИТОГ Г31.5 — recsys_finance_domain_specifics

1 кандидат проверен, **отклонён** (пейволл на пяти адресах, не отказ одного адреса). Добавлены реквизиты 81(12):16761–16781 и `tldr`. Задолженности нет. Канон и новизна не затрагиваются.
