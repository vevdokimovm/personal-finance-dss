...
Источник истины — `.drawio` (XML); PNG — производный артефакт.

🔴 **Превью генерируемых диаграмм рендерятся той же командой** — `python -m
tools.diagrams.generate`, через `rsvg-convert` (SVG → PNG) и Graphviz `dot`
(графы со связями). draw.io для этого не нужен. Прежняя редакция объявляла рендер
невозможным «из-за блокировки CDN Chromium» — это опровергнуто замером 25.09.2026,
оба инструмента установлены и работают.

Превью РИСУЕМЫХ вручную диаграмм по-прежнему экспортируются из draw.io desktop
(File → Export as → PNG, масштаб 1.5): их источник — ручная раскладка, и автоматически
воспроизвести её нечем.
