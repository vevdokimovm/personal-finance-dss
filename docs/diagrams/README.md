...
| Класс | Источник | Кто задаёт содержание |
|---|---|---|
| **Структурные** | код проекта | выводится автоматически |
| **Смысловые, написанные кодом** | `src/*.dot` | человек, но текстом |
| **Рисуемые** | `*.drawio` | человек, мышкой |

Вторая строка появилась 25.09.2026: `04_c4_context`, `05_c4_container`,
`15_forecast_GOST` переписаны на DOT и рендерятся той же командой. Их `.drawio`
сняты — источник теперь `src/*.dot`, иначе XML и превью расходились бы.

**Чем оплачено:** `05_c4_container` десять версий описывала фронт как «Jinja2 +
ванильный JS, 7 страниц» после того, как Jinja была снесена целиком;
`15_forecast_GOST` содержала «оценку тренда b», отменённую каноном v3.10.0;
`04_c4_context` не знала ни одной из четырёх внешних систем (ЦБ РФ, Telegram,
Plaid, почта). Гейт `tests/test_diagrams_match_code.py` теперь ловит и это:
упоминание снесённой технологии — отдельная проверка, потому что оно не молчит
о новом, а УТВЕРЖДАЕТ неверное.
