...
Эталоны платформо- И браузерозависимы (рендер шрифтов отличается и между ОС,
и между движками), поэтому имя файла эталона содержит браузер, а сам
...
Покрытие: 13 публичных страниц × 2 темы (было 14 — `/profile` и `/forgot-password` убраны
вместе с их Jinja-роутами, v8.23.0: React-замена живая, полный цикл проверен вручную в
браузере). Тир `full`: нужен браузер, гоняется перед тегом/релизом. Pillow используется для
диффа; если его нет — тест скипается.
"""
...
# /profile и /forgot-password сняты с покрытия v8.23.0 — их Jinja-роуты снесены (React-замена).
PAGES = [
...
def test_page_matches_baseline(page, base_url: str, path: str, theme: str):
    ...
    # 🔴 Имя эталона включает БРАУЗЕР — без этого мультибраузерный прогон падал
    # на 28 параметрах из 72, и падал ровно месяц. Механика отказа: тир `full`
    # гоняет chromium, firefox и webkit по очереди; первый браузер создавал эталон
    # и пропускался, остальные два сравнивали СВОЙ рендер с ЧУЖИМ и получали 100 %
    # различий — рендер шрифтов и высота полной страницы у движков разные.
    # Локально дефект невидим: в песочнице гоняется один chromium, и механизм
    # «создал эталон → совпало» отрабатывает штатно.
    browser = page.context.browser.browser_type.name if page.context.browser else "unknown"
    baseline_path = os.path.join(SNAPSHOT_DIR, f"{slug}__{theme}__{browser}.png")

    page.set_viewport_size({"width": 1280, "height": 800})
    page.goto(base_url + path, wait_until="networkidle")
    # 🔴 Ключ темы — `fp-theme` и формат Zustand persist, как в SPA. Здесь до v9.13.11
    # стоял `finpilot-theme` — ключ снесённой Jinja-версии: он ничего не значил,
    # и «тёмный» снимок снимался в светлой теме. Тот же дефект в соседнем файле
    # (`test_a11y_axe.py`) починили ещё в v8.45.0 — здесь он дожил до сегодня,
    # потому что этот тест в обычный прогон не входит и падал по другой причине.
    page.evaluate(
