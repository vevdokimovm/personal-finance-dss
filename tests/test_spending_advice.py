...
class TestAnalyze:
    def test_baseline_is_median_of_past_months(self):
        ...
        records = _months("Кафе и рестораны", {"2026-01": 10,
                          "2026-02": 12, "2026-03": 8, "2026-04": 30})
    ...
    def test_anomaly_flagged(self):
        ...
        records = _months("Покупки", {"2026-01": 100, "2026-02": 100,
                          "2026-03": 100, "2026-04": 999})
        stats = SpendingAdvisor().analyze(records, current_period="2026-04")
    ...
    def test_anomaly_with_spread(self):
        ...
        records = _months("Развлечения", {"2026-01": 100,
                          "2026-02": 120, "2026-03": 80, "2026-04": 500})
        stats = SpendingAdvisor().analyze(records, current_period="2026-04")
    ...
    def test_low_txn_count_skipped(self):
        ...
        records += _months("Прочее", {"2026-01": 5, "2026-02": 5,
                           "2026-03": 5, "2026-04": 5}, txns_per_month=2)
        stats = SpendingAdvisor().analyze(records, current_period="2026-04")
        assert all(s.category != "Покупки" for s in stats)

    def test_pain_score_orders_results(self):
        records = (
            _months("Кафе и рестораны", {"2026-01": 200, "2026-02": 200,
                    "2026-03": 200, "2026-04": 200}, txns_per_month=10)
            + _months("Продукты", {"2026-01": 1000, "2026-02": 1000,
                      "2026-03": 1000, "2026-04": 1000}, txns_per_month=4)
        )
        stats = SpendingAdvisor().analyze(records, current_period="2026-04")
...
class TestGenerateAdvice:
    def test_overspend_advice(self):
        records = _months("Развлечения", {"2026-01": 1000, "2026-02": 1200,
                          "2026-03": 800, "2026-04": 5000}, txns_per_month=3)
    ...
    def test_obligatory_category_excluded(self):
        ...
        records = _months("ЖКХ и связь", {"2026-01": 5000, "2026-02": 5200,
                          "2026-03": 4800, "2026-04": 20000}, txns_per_month=2)
        advice = SpendingAdvisor().generate_advice(records, current_period="2026-04")
    ...
    def test_min_saving_filter(self):
        ...
        records = _months("Подписки и сервисы", {
                          "2026-01": 300, "2026-02": 300, "2026-03": 300, "2026-04": 300}, txns_per_month=3)  # noqa: E501
        advice = SpendingAdvisor().generate_advice(records, current_period="2026-04")
    ...
    def test_top_k_limit(self):
        ...
        for cat in ["Кафе и рестораны", "Развлечения", "Покупки", "Транспорт"]:
            records += _months(cat, {"2026-01": 8000, "2026-02": 8000,
                               "2026-03": 8000, "2026-04": 8000}, txns_per_month=10)
