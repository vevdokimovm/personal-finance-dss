...
SCENARIOS = [
    (
        ...
        # v3.1.0 (G6): подушка 0.57 мес < floor — сначала стартовый месяц
        # ликвидности (30к), остаток в дорогой долг (20% > bench).
        dict(
            best_name="Акцент: резерв (10/90/0)",
            x=(3500.0, 31500.0, 0.0),
            utility=0.3207,
            ind_rt_lt_dt=(35000.0, 0.5714, 0.125),
            top3=["Акцент: резерв (10/90/0)", "Всё в резерв", "Акцент: резерв (0/90/10)"],
        ),
    ),
    (
        "no_debt_aggressive",
        ...
        # v3.1.0 (G6): даже агрессивный профиль сначала добирает 1 месяц
        # ликвидности (кейс SP-00299 экспертизы), остаток — в цель.
        dict(
            best_name="Акцент: резерв (0/60/40)",
            x=(0.0, 30000.0, 20000.0),
            utility=0.72,
            ind_rt_lt_dt=(50000, 0.4, 0.0),
            top3=["Акцент: резерв (0/60/40)", "Акцент: резерв (0/70/30)", "Акцент: резерв (0/80/20)"],  # noqa: E501
        ),
    ),
    (
        "cheap_debt_redirect",
