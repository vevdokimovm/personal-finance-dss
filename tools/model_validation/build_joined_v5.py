"""Сборка joined раунда 5: модель v3.5.0 + четыре эксперта на датасете v5.

Контракт наследуется от раунда 4 без изменений (позиционное сопоставление,
enum статуса и доминанты, бюджет, `confidence`, action-vector) — это условие
сравнимости раундов между собой. Отличия только в данных:

  * генератор и seed — `PortraitGeneratorV5`, 20260723 (сборка 2), префикс SP5;
  * `LUMP_ACTION_MAP` дополнен словарями действий четвёрки раунда 5. Ни один
    из четырёх не назначил разовый ход в резерв — карта это допускает, но
    факт стоит держать в голове при чтении метрики.

Выход: knowledge/model_validation/joined_v5.csv.gz.
"""
...
LUMP_ACTION_MAP: dict[str, str] = {
    ...
    # --- словари раунда 5 ---
    "close_debt_to_free_payment": "debt",
    "close_expensive_debt": "debt",
    "close_toxic_debt": "debt",
    "prepay_expensive_debt": "debt",
    "prepay_toxic_debt": "debt",
    "prepay_debt_partial": "debt",
    "pay_down_debt_from_savings": "debt",
    "deploy_surplus_to_investments": "goals+",
    "fund_goal_from_savings": "goals+",
    "top_up_goal": "goals+",
    # --- резерв ---
...
def build(outcomes_path: Path, experts: dict[str, Path],
          ...
          seed: int = 20260723, n: int = 12000) -> dict:
    gen = PortraitGeneratorV5(seed, n=n)
