...
def main() -> None:
    ...
    # 4. Меняется ли совет
    print(f"\n{C}Меняется ли ТОП-3 совет между 10% и 5%?{X}")
    s10 = _top3_signatures(0.10)
    s05 = _top3_signatures(0.05)
    print(f"  топ-3 @10% (x_obl,x_res,x_goals): {s10}")
    print(f"  топ-3 @ 5% (x_obl,x_res,x_goals): {s05}")
    same = s10 == s05
    print(f"  → {G+'совет ИДЕНТИЧЕН'+X if same else Y+'совет ОТЛИЧАЕТСЯ'+X}")
