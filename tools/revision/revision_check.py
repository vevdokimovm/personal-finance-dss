...
SKIP_DIRS = frozenset({
    ...
    # Тот же класс гейта-с-ложной-уверенностью, что dist/ выше — но чужое дерево, не
    # сборочный артефакт: `_base/` инжектируется системой «база» на 63 репозитория
    # (см. .repo-id/.repo-meta), внутренние ссылки в нём (`scripts/pack_release.py`,
    # `docs/PHILOSOPHY.md` и т.п.) рассчитаны на СВОЙ корень базы, не на REPO_ROOT этого
    # репозитория — резолвятся сюда как несуществующие пути и дают систематические ложные
...
LEGACY_ALLOWLIST_FILES = frozenset({
    ...
    # 🔴 Первичный материал исследования (§9), добавлено 11.09.2026: «95% CI» в этих файлах —
    # описание методики ЦИТИРУЕМЫХ научных работ (доверительные интервалы их оценок), а не
    # параметр нашей модели. Паттерн заведён против утечки v2.x в живые доки и здесь ловит
    # чужой домен — тот же случай, что `tools/survey_analysis/report.py` строкой выше.
...
# rel-путь -> обоснование легитимного CJK (замороженные файлы попадают сюда
# с объяснением, историю не переписываем).
CJK_ALLOWLIST: dict[str, str] = {
    # 🔴 Первичный материал исследования рынка (правило §9): дословные ответы агентов
    # с названиями китайских финансовых продуктов (Alipay, Suishouji, Wacai — приведены
    # в цитате в оригинальном написании). Это НЕ токен-глюк, против которого заведена
    # канарейка, а имя собственное во внешней цитате: сокращать его до транслитерации
    # значит править первичный материал, чего правило §9 не допускает — сырьё хранится
    # дословно. Тот же класс, что `dist/` в SKIP_DIRS выше: гейт верен по замыслу и ловит
    # легитимное, потому что замысел писался до появления такого содержимого в дереве.
    # 🔴 Сами иероглифы здесь не приводятся — канарейка не должна ловить саму себя,
    # об этом сказано в её собственном комментарии выше. Поймано на первом же прогоне.
    "docs/research/raw/competitors_recommendation_engines_2026-09-08.md":
        "названия китайских финпродуктов в дословной цитате исследования (§9)",
    # 🔴 Тот же класс, добавлено 11.09.2026: корейские первоисточники по APAC-необанкам.
    # Здесь не только имена продуктов, но и дословные тексты дисклеймеров с сайта
    # KakaoBank — ровно тот материал, за формулировками которого тема и ходила
    # (формула «справочного характера» в оригинале). Перевод без оригинала
    # обесценивает довод:
    # проверить формулировку станет нельзя.
    "docs/research/raw/tails_cbr_mr_software_registry_2026-09-10.md":
        "дословные корейские дисклеймеры и названия сервисов KakaoBank/Toss (§9)",
    "docs/research/raw/banks_apac_neobanks_2026-09-10.md":
        "названия сервисов и цитаты из корейских и японских первоисточников (§9)",
    # 🔴 Реестр тем несёт ТЕ ЖЕ дословные цитаты, что и сырьё, и по той же причине:
    # находка тем 7 и 24-хв — именно ФОРМУЛИРОВКА дисклеймеров Toss и KakaoBank
    # («оптимум не обещан», «справочного характера»), а не их пересказ. Рядом с каждой
    # стоит перевод; убрать оригинал — значит сделать вывод непроверяемым, оставив
