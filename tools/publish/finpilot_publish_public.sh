...
OPTIONAL_DOCS=(
  # math_model.md                 # математическая модель — ядро алгоритма (IP)
...
# 🔴 Тем же правилом отсекаются тесты ХУКОВ рабочей станции: хуки живут в `.claude/`,
# этого контура в зеркале нет вовсе, и тест на него — ссылка в никуда. До v9.13.3
# такие файлы уезжали в зеркало и держали гейт внутренней кухни красным (ВЛ-29):
# они по построению цитируют решения владельца и называют вахтенный журнал.
collect_internal_tool_tests() {
  ...
  "$GREP" -rlE '(from|import) tools[. ]|["'"'"']tools\.|\.claude' "$SOURCE_REPO/tests" 2>/dev/null \
