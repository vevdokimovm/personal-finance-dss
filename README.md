...
<!-- STATUS -->
> **Сейчас:** `v8.29.3` · 2026-08-29 · Гейт CLEAN: третий источник состояния убран, точка входа 1424 → 44 строки
...
![tests](https://img.shields.io/badge/tests-1629%20%2B%20225-brightgreen)
...
## Текущее состояние (v8.28.0, август 2026)

| | |
|---|---|
| **Матмодель** | канон **v3.9.0**, пять раундов независимой экспертной сертификации, оценка по десяти шкалам — **87.2 / 100** |
| **Тесты** | бэкенд **1629** на SQLite и матрице PostgreSQL, фронт Vitest **225**, гейт покрытия 90%, flake8 = 0, ревизионный гейт docs↔code |
| **Вехи** | 1–7 закрыты. **8 (фронтенд) идёт** — React 19 SPA заменил Jinja2 на основных экранах (дашборд, планирование, операции, обязательства, цели, банки, профиль), включая создание/правку/удаление обязательств, ликвидных активов, операций и целей (мягкое удаление с отменой; у целей — отдельное действие «внести прогресс», не общий edit); бюджеты — тот же CRUD-паритет следующей партией. Прод-деплой пайплайн (Docker + nginx) проверен на изолированном стенде |
