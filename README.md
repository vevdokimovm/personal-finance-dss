...
<!-- STATUS -->
> **Сейчас:** `v8.30.2` · 2026-09-02 · `POST /demo/clear` больше не стирает данные авторизованного
...
![version](https://img.shields.io/badge/version-8.30.2-blue)
...
## Текущее состояние (v8.30.2, сентябрь 2026)

| | |
|---|---|
| **Матмодель** | канон **v3.9.0**, пять раундов независимой экспертной сертификации, оценка по десяти шкалам — **87.2 / 100** |
| **Тесты** | бэкенд **1632** на SQLite и матрице PostgreSQL, фронт Vitest **238**, гейт покрытия 90%, flake8 = 0, ревизионный гейт docs↔code |
| **Вехи** | 1–7 закрыты. **8 (фронтенд) идёт** — React 19 SPA заменил Jinja2 на основных экранах (дашборд, планирование, операции, обязательства, цели, банки, профиль); CRUD-паритет §8.2 закрыт полностью (5/5): обязательства, ликвидные активы, операции, цели (мягкое удаление с отменой; у целей — отдельное действие «внести прогресс», не общий edit) и бюджеты (upsert по категории). Прод-деплой пайплайн (Docker + nginx) проверен на изолированном стенде |
