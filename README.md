...
<img src="knowledge/brand/logo_reference.png" alt="FINPILOT" width="180">
...
<!-- STATUS -->
> **Сейчас:** `v9.13.28` · 2026-09-24 · веха 9 открыта, синтез корпуса завершён (12 документов из 12), идёт предподготовка к деплою.
...
![version](https://img.shields.io/badge/version-9.13.28-blue)
![python](https://img.shields.io/badge/python-3.13-blue)
![coverage](https://img.shields.io/badge/coverage-gate%2090%25-brightgreen)
![tests](https://img.shields.io/badge/tests-2456%20%2B%20881-brightgreen)
...
## Текущее состояние (v9.13.28, сентябрь 2026)
...
| **Тесты** | бэкенд **2456** (SQLite и матрица PostgreSQL), фронт Vitest **881**, мультибраузерные E2E **213** (chromium + firefox + webkit), фронтовые E2E **201**, визуальная регрессия и live-a11y **48**, стресс-property тысячами примеров + мутации. Все три тира (`fast`, `full`, `deep`) гоняются на КАЖДЫЙ push. Гейт покрытия 90 % (ядро — 95 %), flake8 = 0, mypy чист, pylint ≥ 9.6, bandit и pip-audit чисты |
...
Полный комплект — `docs/diagrams/` (20 схем). **Каждая несёт подпись «код vX.Y.Z · канон
матмодели vA.B.C»**: без неё нельзя отличить проверенную схему от забытой, и именно так
`05_c4_container` десять версий описывала снесённый Jinja2, а схема прогноза — отменённый
каноном тренд.

Десять диаграмм **генерируются** командой `python -m tools.diagrams.generate` — из кода
(таблицы, роутеры, сервисы, импорты ядра) или из описания на DOT. Актуальность сторожит
гейт `tests/test_diagrams_match_code.py`.

### Контейнеры (C4 Level 2)

![Контейнеры](docs/diagrams/05_c4_container.drawio.png)

### Главный пайплайн расчёта (ГОСТ 19.701-90)

![Пайплайн](docs/diagrams/01_main_pipeline_GOST.drawio.png)

### База данных: 31 таблица, 33 связи

![ER](docs/diagrams/10_er_database.drawio.png)

### Прогноз: SES без тренда + Монте-Карло

![Прогноз](docs/diagrams/15_forecast_GOST.drawio.png)

Остальные: контекст системы, компоненты, зависимости ядра, Avalanche-фильтр, взвешенная
обеспеченность целей, цикл планирования, импорт выписки, жизненный цикл альтернативы,
ORM-классы, IDEF0, варианты использования, PDCA, BPMN, EPC, VSM, DMAIC.
...
юр-контур входа/регистрации закрыт. **CRUD-паритет закрыт полностью (5/5):** до v8.24
React-экраны пяти финансовых сущностей (операции, обязательства, цели, активы, бюджеты)
были read-only, и Jinja оставалась единственным способом что-то ввести или изменить.
Все пять получили создание, правку и мягкое удаление с отменой, после чего Jinja-слой
снесён целиком — каталога `app/templates` больше нет. Закрыты и юр-контур согласий
на всём фронте (Э6), и security/perf-гейты (CSP, security-заголовки, Э7). План —
...
Интерфейс целиком на React SPA — Jinja-слой снесён в вехе 8, каталога `app/templates`
больше нет. **Смотреть фронт локально нужно через Vite dev-сервер, не через
`docker compose up`** — dev-compose поднимает только бэкенд.
...
```
Backend будет доступен на `http://localhost:8000` (API — на `/api/*`,
Swagger — на `/docs`, собранный SPA — в корне).
