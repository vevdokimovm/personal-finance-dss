            ...
            <li key={type} className="fp-consents__item">
              ...
              {document && (
                <p className="fp-consent-doc-link">
                  ...
                  {/* 🔴 По 152-ФЗ согласие даётся на КОНКРЕТНУЮ редакцию. Экран печатал
                      только ту, на которую человек согласился, и не отличал её
                      от действующей: расхождение считалось на бэкенде и не доезжало
                      никуда. `is_current` может отсутствовать в ответе, собранном
                      до v9.6.0, — тогда ничего лишнего не показываем. */}
                  {state.is_current === false && state.current_version && (
                    <span className="fp-consents__version fp-consents__version--stale">
                      {t(" · действует ред. {version}", {
              ...
              <div className="fp-profile__actions">
                {/* 🔴 Подтверждение новой редакции и отзыв — РАЗНЫЕ действия,
                    а не альтернативы. Первая редакция этого блока ставила их в один
                    тернарник, и ветка подтверждения поглощала ветку отзыва: пока
                    редакция устарела, отозвать согласие из интерфейса было нельзя
                    вообще. Условие писалось «под personal_data» (у него отзыва нет
                    по построению) и типов не различало.

                    Шапка этого файла описывает ровно такой дефект как уже случившийся
                    в v8.41.0 — «отозвать маркетинговое было невозможно». Он вернулся
                    условно, по флагу расхождения, и был найден четвёртым проходом
                    независимого аудита в тот же день. Регулятор читает такое как
                    понуждение к повторному согласию: чтобы отозвать — сначала
                    согласись заново. */}
                {state.granted && state.is_current === false && (
                  <Button
                    variant="primary"
                    aria-disabled={grant.isPending}
                    aria-busy={grant.isPending}
                    aria-label={t("Подтвердить новую редакцию: {name}", {
                      name: label(type),
                    })}
