    ...
    coverage: {
      ...
      exclude: [
        ...
        /* 🔴 Файлы маршрутов исключены ПО СВОЙСТВУ, а не ради процента.
           ...
           Проверено: у `routes/transactions.tsx` (6 строк, одна декларация) счётчик
           показывает 5 ветвлений, все в колонке 57 первой строки — их там не писал
           никто.
           🔴 Наша логика в маршрутах — `validateSearch` — покрыта отдельно
           (`src/routes/routes.test.ts`), и проверка полноты набора маршрутов
           там же. Исключение без этого теста было бы именно накруткой. */
        "src/routes/**",
      ],
