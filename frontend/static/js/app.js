...
async function renderCategoryRules() {
    ...
    try {
        ...
        box.innerHTML = rules.map(r => `<div class="table-row" style="display:flex; justify-content:space-between; align-items:center; gap:10px;">
            ...
            <button class="ghost-button rule-del" data-rule-id="${r.id}" style="padding:4px 8px; color:var(--c-red-text); font-size:.85rem;" title="Удалить правило" aria-label="Удалить правило">✕</button>
        </div>`).join('');
