...
function renderTop3Card(a, rank, indicators, weights) {
    ...
    <article class="glass-panel alt-card" style="border:1px solid ${bc}; padding:20px 22px; margin-bottom:16px; cursor:pointer;" title="Нажмите, чтобы раскрыть разбор">
        ...
        ${renderCalcDetails(a, indicators, weights)}
    </article>`;
}

// ── Раскрываемый расчёт альтернативы (детали формул) ──────────
function renderCalcDetails(a, ind, weights) {
    const It = ind.It || 0, Et = ind.Et || 0, SigmaP = ind.SigmaP || 0, Rt = ind.Rt || 0;
    const m = v => fmt.cur(v);
    const ltRow = a.Lt_new !== undefined ? `
        <div class="calc-row"><span class="calc-label">Ликвидность Lt'</span>
            <span>свободные деньги ÷ обязательные платежи (расходы + кредиты) = <b>${a.Lt_new.toFixed(3)}</b></span></div>` : '';
    const dtRow = a.Dt_new !== undefined ? `
        <div class="calc-row"><span class="calc-label">Долговая нагрузка Dt'</span>
            <span>платежи по кредитам ÷ доход = <b>${(a.Dt_new * 100).toFixed(1)}%</b></span></div>` : '';

    // Разбор оценки U: вклад каждого критерия = вес профиля × нормированное значение
    let breakdown = '';
    if (a.scores && weights) {
        const crit = [
            { label: 'Свободные деньги', w: weights.w_rt, n: a.scores.Rt_norm },
            { label: 'Ликвидность', w: weights.w_lt, n: a.scores.Lt_norm },
            { label: 'Снижение долга', w: weights.w_dt, n: a.scores.Dt_norm },
            { label: 'Цели', w: weights.w_goals, n: a.scores.Si_norm },
        ];
        const rows = crit.map(c => `
            <div style="display:flex; justify-content:space-between; gap:8px; font-size:.77rem; padding:3px 0;">
                <span style="color:var(--c-text2);">${c.label}</span>
                <span style="color:var(--c-text3); white-space:nowrap;">${c.n.toFixed(2)} × вес ${c.w.toFixed(2)} = <b style="color:var(--c-text);">${(c.w * c.n).toFixed(3)}</b></span>
            </div>`).join('');
        const dominant = crit.reduce((x, y) => (y.w * y.n > x.w * x.n ? y : x));
        breakdown = `
        <div class="calc-row" style="margin-top:4px;">
            <span class="calc-label">Из чего сложилась оценка U = ${a.utility}</span>
            <div style="margin-top:6px; padding:8px 12px; background:rgba(255,255,255,.03); border-radius:8px;">
                ${rows}
                <div style="display:flex; justify-content:space-between; font-size:.77rem; padding-top:6px; margin-top:4px; border-top:1px solid var(--c-border);">
                    <span style="color:var(--c-text2);">Итог</span><span><b>${a.utility}</b> из 1.00</span>
                </div>
            </div>
        </div>
        <div class="calc-note">Каждый критерий приводится к шкале 0–1 (сравнением со всеми альтернативами) и умножается на вес вашего профиля. Наибольший вклад здесь дал критерий «${dominant.label}» — по нему этот план обходит остальные.</div>`;
    }

    return `
    <details class="calc-details">
        <summary>Как алгоритм пришёл к этому</summary>
        <div class="calc-body">
            <div class="calc-row"><span class="calc-label">Шаг 1 · Свободные деньги (Rt)</span>
                <span>${m(It)} доход − ${m(Et)} расходы − ${m(SigmaP)} платежи по кредитам = <b>${m(Rt)}</b></span></div>
            <div class="calc-row"><span class="calc-label">Шаг 2 · Распределение этой суммы</span>
                <span>на досрочку <b>${m(a.x_obligations)}</b> · в резерв <b>${m(a.x_reserve)}</b> · на цели <b>${m(a.x_goals)}</b></span></div>
            ${ltRow}
            ${dtRow}
            ${breakdown}
        </div>
...
function renderPlanning(res) {
    ...
    if (inp) {
        ...
        const rowI = (label, val) => `<div style="display:flex; justify-content:space-between; gap:8px; padding:4px 0;"><span style="color:var(--c-text3);">${label}</span><strong style="color:var(--c-green); white-space:nowrap;">${val}</strong></div>`;
        const rowE = (label, val) => `<div style="display:flex; justify-content:space-between; gap:8px; padding:4px 0;"><span style="color:var(--c-text3);">${label}</span><strong style="color:var(--c-red); white-space:nowrap;">${val}</strong></div>`;
        inp.innerHTML = `
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; font-size:.8rem;">
                <div style="border:1px solid rgba(74,222,128,.35); border-radius:var(--r-sm); padding:10px 12px; background:rgba(74,222,128,.05);">
                ...
                <div style="border:1px solid rgba(248,113,113,.35); border-radius:var(--r-sm); padding:10px 12px; background:rgba(248,113,113,.05);">
    ...
    if (optC) {
        if (res.top3 && res.top3.length) {
            optC.innerHTML = `
                ...
                ${res.top3.map((a, i) => renderTop3Card(a, i, res.indicators, res.weights)).join('')}`;
        } else {
            optC.innerHTML = `
