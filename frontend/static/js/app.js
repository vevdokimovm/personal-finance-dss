...
function renderObligations() {
    ...
    list.innerHTML = state.obligations.map(o => {
        // Прогресс по сроку: сколько месяцев уже выплачено из общего срока.
        let paidBlock = '';
        if (o.start_date && o.term > 0) {
            const start = new Date(o.start_date);
            const now = new Date();
            const elapsed = Math.max(0, (now.getFullYear() - start.getFullYear()) * 12 + (now.getMonth() - start.getMonth()));
            const paidMonths = Math.min(elapsed, o.term);
            const pct = Math.min(100, paidMonths / o.term * 100);
            const paidSum = paidMonths * pn(o.monthly_payment);
            paidBlock = `
                ...
                <div style="display:flex; justify-content:space-between; font-size:.76rem; color:var(--c-text3); margin-top:4px;">
                    <span>Выплачено ${paidMonths} из ${o.term} мес (~${fmt.cur(paidSum)})</span>
                    <span>Осталось ${o.term - paidMonths} мес</span>
                </div>`;
        }
        const takenLine = o.start_date
            ? `<span style="color:var(--c-text3);"> · взят ${fmt.date(o.start_date)}</span>`
            : '';
        return `
        <article class="stack-item" style="flex-direction:column; align-items:stretch; gap:0;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    <div class="stack-item-title">${esc(o.name)}</div>
                    <div class="stack-item-text">
                        ${fmt.cur(o.monthly_payment)} / мес · Ставка ${o.interest_rate}% · Срок ${o.term} мес${takenLine}
