...
def cmd_move(name: str) -> None:
    ...
    **Что остаётся под охраной — правило 1.** Единственный реальный риск здесь: затереть
    ...
    print(f"{name} → {MAP[name]}: перенесено {moved} · коллизий {collided} · секретов в карантин {secrets}")
    for line in log[:15]:
        print("  " + line)
    if len(log) > 15:
