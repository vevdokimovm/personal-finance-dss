...
def main() -> int:
    ...
    if a.update_readme:
        ...
        # 🔴 02.09.2026, заказ владельца дословно: «что за рандом заметки
        # и багнутая опять структуруа добавилоась». Различение 29.08 было
        # верным для `product` (README проекта — структура репозитория там
        # уместна), но неверным для `profile`: витрина ЧЕЛОВЕКА для найма —
        # не репозиторий, и таблица «reports/ · 2 файла» на ней читается как
        # мусор. Различение третьего слоя: в `profile` блок не заводится
        # и не обновляется — его там не должно быть вовсе, а не «пусть висит,
        # раз уж завёлся». `--force` остаётся ручным обходом для владельца.
        if repo_class == "profile" and not a.force:
            print(
                f"{repo.name}: .repo-class = profile — витрина человека, "
                f"не репозиторий. Блок «Структура» здесь не заводится и не "
                f"обновляется: перечень папок на странице найма — служебный "
                f"шум. Нужен всё равно — добавь --force."
            )
            return 2

        if repo_class == "product" and not a.force and not has_block:
            print(
