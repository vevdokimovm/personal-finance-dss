...
ПЯТЬ КЛАССОВ МЁРТВОЙ ПРОВЕРКИ, каждый невидим по-своему:
  ...
  5. **Проверка без докстроки.** Не сообщает даже того, что делает, — и тем
     более не называет, какой класс ошибки она принципиально НЕ поймает
     (`71` §7г-бис). Добавлено 29.08.2026: пять проверок из 26 были немыми.
...
def check_checks_documented() -> list[str]:
    """Класс 5: проверка без докстроки — она не сообщает даже того, что делает.

    🔴 Заведено 29.08.2026 по кандидату №4 очереди внедрения
    (`reports/research/paradigms-and-contracts-2026-08-29.md`): каждая проверка
    обязана объявлять, какой класс ошибки она **принципиально не поймает**.
    Расширяет `71` §7г-бис с инструментов на сами проверки.

    Зачем это мета-гейту, а не гейту: молчащая проверка — это не дефект репы,
    а дефект инструмента, и ловить его должен тот, кто следит за инструментами.

    Проверяется наличие документации, НЕ её качество: «что не поймает»
    формулирует человек, и подделать эту строку легко. Но пустая докстрока
    подделкой не является — её просто нет.
    """
    tree = ast.parse(GATE.read_text(encoding="utf-8"))
    problems = []
    for node in ast.walk(tree):
        ...
        if not doc.strip():
            problems.append(f"проверка без докстроки: {node.name}() — "
                            f"не сообщает даже того, что делает")
    return problems
...
def main() -> int:
    ...
    blocks = [
        ...
        ("Проверки без документации", check_checks_documented()),
