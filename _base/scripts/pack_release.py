...
ПРЕДУСЛОВИЯ (проверяются до сборки):
  · корень репы существует, в нём есть `README.md` и `VERSION`;
  · `VERSION` разбирается как `X.Y.Z` — имя архива строится из неё;
  · каталог назначения доступен на запись.

ПОСТУСЛОВИЯ (проверяются после — архив, который не разворачивается,
хуже отсутствующего, потому что выглядит сделанным):
  · архив существует и открывается как zip;
  · в корне ровно ОДНА папка-обёртка `<repo>-vX.Y.Z/`;
  · внутри есть `README.md` и `VERSION`, и `VERSION` совпадает с именем;
  · мусора macOS (`.DS_Store`, `__MACOSX`, `._*`) нет;
  · у каждой записи выставлен UTF-8-флаг — иначе кириллические имена
    рассыпаются при распаковке в другой локали.

ИНВАРИАНТ: имя архива, `VERSION` внутри него и строка в `LEDGER.tsv`
называют одну и ту же версию. 🔴 У реп нет локального `.git`, поэтому
архив — не копия версии, а её единственный экземпляр (`/auto` §2).

🟡 **Уточнено 07.09.2026:** у `base-repo` локальный `.git` заведён — у неё
одной из 67. Правило «архивы не удаляются» этим НЕ отменяется, и вот почему:
история базы живёт на GitHub и подтягивается оттуда, но у остальных 66 реп
её нет вовсе, а собранные архивы прошлых версий остаются единственным
носителем в любом случае. Отменять правило можно будет только тогда, когда
история появится у всех — и это решение владельца, а не следствие одной репы.
"""
import datetime
import hashlib
import atexit
