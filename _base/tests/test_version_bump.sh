...
#
# 🔴 С 03.09.2026 способов ДВА, и предпочтителен объявленный.
# `BASE_ARTIFACTS` — штатная переменная (`scripts/_roots.py`), которую читают
# и Python-скрипты, и `deploy.sh`. Тест, опирающийся на объявленный механизм,
# проверяет то же, что работает в бою; подмена `HOME` проверяла обходной путь.
#
# Подмена `HOME` ОСТАВЛЕНА второй линией: если переменную однажды перестанут
# читать, вывод уедет в песочный дом, а не в настоящую папку владельца.
# Первый прогон этого теста положил архив в РЕАЛЬНЫЙ `~/Downloads` — цена
# ошибки здесь уже заплачена, и защит должно быть две.
export HOME="$SANDBOX/fakehome"
export BASE_ARTIFACTS="$HOME/Developer"
...
if printf '%s' "$out" | grep -q "собран"; then
  ok "архив собран"
  [ -f "$BASE_ARTIFACTS/rel-alpha-v1.0.0.zip" ] \
    && ok "имя архива по канону <репа>-vX.Y.Z.zip" \
    || bad "имя архива по канону" "$(ls "$BASE_ARTIFACTS" 2>&1)"
...
# и загадил его.
# 🔴 Проверяются ОБЕ настоящие папки: артефакты переехали в `~/Developer`,
# но `~/Downloads` остаётся местом, куда промах записал бы файл при откате
# правки. Проверка только новой папки пропустила бы возврат старого поведения.
[ ! -f "$REAL_HOME/Developer/rel-alpha-v1.0.0.zip" ] \
  && ok "настоящий ~/Developer не тронут" \
  || bad "настоящий ~/Developer не тронут" "тест негерметичен — убрать файл вручную"
...
# =============================================================================
case_ V7 "ВРЕМЯ, а не только дата — во всех трёх местах (94 §3)"
# =============================================================================
# 🔴 ЗАКАЗ ВЛАДЕЛЬЦА 03.09.2026 21:29: «фиксируй в документах не только дату,
# но и время». Повод арифметический: за сутки система закрывает до десяти
# версий, и по одной дате порядок событий внутри дня не восстановить.
#
# Проверяются ВСЕ три места, которые правит подъём, — иначе время появится
# в одном и разойдётся с остальными, а это ровно `PIT-G`.
S="$(make_repo stamp-alpha 3.0.0)"
body | python3 "$BUMP" stamp-alpha --minor --title "проверка времени" --body-stdin >/dev/null 2>&1

grep -qE '^## \[3\.1\.0\] — [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2} —' "$S/CHANGELOG.md" \
  && ok "CHANGELOG: секция несёт дату И время" \
  || bad "CHANGELOG: секция несёт дату и время" "$(grep -m1 '^## \[3\.1\.0\]' "$S/CHANGELOG.md")"

grep -qE '\*\*Дата:\*\* [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}' "$S/WATCHLOG.md" \
  && ok "WATCHLOG §0: дата и время" \
  || bad "WATCHLOG §0: дата и время" "$(grep -m1 'Дата:' "$S/WATCHLOG.md")"

grep -qE '`v3\.1\.0` · [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2} ·' "$S/README.md" \
  && ok "README-статус: дата и время" \
  || bad "README-статус: дата и время" "$(grep -m1 'Сейчас:' "$S/README.md")"

# 🔴 ГЛАВНОЕ РАЗЛИЧАЮЩЕЕ: повторный подъём. Старая регулярка съедала только
# `\d{4}-\d{2}-\d{2}` и упиралась в ` 21:38 · ` — замена молча не срабатывала,
# оставляя репу в состоянии «версия новая, витрина старая» ПОСЛЕ подъёма.
body | python3 "$BUMP" stamp-alpha --patch --title "второй подъём" --body-stdin >/dev/null 2>&1
grep -qE '`v3\.1\.1` · [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2} · второй подъём' "$S/README.md" \
  && ok "повторный подъём перезаписал статус со временем" \
  || bad "повторный подъём перезаписал статус" "$(grep -m1 'Сейчас:' "$S/README.md")"

# Легаси: запись без времени законна и нарушением не становится.
printf '# CHANGELOG\n\n## [1.0.0] — 2026-08-01 — старая запись (MINOR)\n\nтело\n' \
  > "$SANDBOX/legacy-CHANGELOG.md"
python3 - "$BASE/scripts" "$SANDBOX/legacy-CHANGELOG.md" <<'PYEOF' && \
  ok "запись БЕЗ времени по-прежнему разбирается (легаси)" || \
  bad "запись без времени разбирается"
import re, sys
line = open(sys.argv[2], encoding="utf-8").read().splitlines()[2]
rx = r"^##\s*\[[^\]]+\]\s*[—-]\s*\d{4}-\d{2}-\d{2}(?:\s+\d{2}:\d{2})?\s*[—-]\s*(.+?)\s*$"
sys.exit(0 if re.match(rx, line) else 1)
PYEOF
