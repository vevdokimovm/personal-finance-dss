...
ok(){ [ "${CASE_ACTIVE:-1}" = "0" ] && return 0;   printf '  \033[32m✓\033[0m %s\n' "$1"; PASS=$((PASS+1)); grp_hit G_OK; }
bad(){ [ "${CASE_ACTIVE:-1}" = "0" ] && return 0;  printf '  \033[31m✗ %s\033[0m\n' "$1"; [ -n "${2:-}" ] && printf '      %s\n' "$2"; FAIL=$((FAIL+1)); grp_hit G_BAD; }
...
G_CUR=""; G_LIST=""
# ─── ФИЛЬТР ПО ЗОНЕ И КЕЙСУ (`ROADMAP` §P2, `PIT-039`) ─────────────────────────
# 🔴 ПОВОД: прогон целиком ~3 минуты, и медленный набор начинают пропускать —
# то есть проверка, которую не запускают, перестаёт быть проверкой.
#
#   TZONE=C bash tests/test_deploy.sh       только зона C (13 кейсов)
#   TCASE=C12 bash tests/test_deploy.sh     один кейс
#   TCASE="A1 B3" TZONE=G …                 объединение: и перечисленные, и зона
#
# 🔴 ИМЕНА `TCASE`/`TZONE`, А НЕ `ONLY`/`ZONE` — и это не вкус.
# `ONLY` — **переменная самого деплойера** («обработать только эти репы»,
# `deploy.sh` §ПЕРЕКЛЮЧАТЕЛИ). Тест её экспортирует, деплой читает — и ищет
# репу с именем `A1`, которой нет. Результат: «архивов к разбору: 1»,
# затем «публиковать нечего», **без единой строки о причине**.
#
# Стоило часа отладки: кейс `A1` падал семью проверками, и выглядело это как
# «фильтр сломал тест» или «набор не изолирован». Ни то, ни другое —
# столкновение имён через окружение, которое `bash -n` не видит,
# а `grep` по тесту не находит, потому что вторая половина в другом файле.
#
# **Правило:** переменная окружения, которую тест передаёт проверяемому
# скрипту, обязана быть проверена на столкновение С ЕГО СОБСТВЕННЫМИ ключами.
#
# 🔴 ФИЛЬТР РЕЖЕТ ТЯЖЁЛОЕ, А НЕ ТОЛЬКО ПРОВЕРКИ. Вся работа идёт через
# `run_deploy` (122 вызова) — он и пропускается. Резать одни `assert`
# было бы обманом: вывод «прошло за 5 с» при тех же трёх минутах работы.
#
# 🔴 ГРАНИЦА, НАЙДЕННАЯ ПЕРВЫМ ЖЕ ПРОГОНОМ ФИЛЬТРА (04.09.2026):
# **НАБОР НЕ ИЗОЛИРОВАН ПО КЕЙСАМ.** `TZONE=A` даёт два падения — не из-за
# фильтра, а потому что кейс `A2` читает `$SANDBOX/c2`, созданный в `G1`,
# то есть в ДРУГОЙ зоне.
#
# **Проверено обоими исходами:** `TZONE=A` → 25 прошло, 2 упало;
# `TZONE=AG` → **92 прошло, ноль падений**. Диагноз подтверждён, а не выведен.
#
# Замер общих каталогов: `c2`, `c22`, `t1`, `t2`, `t22`, `t43` —
# **шесть** используются более чем одним кейсом.
#
# Это не дефект фильтра, а свойство набора, которое фильтр СДЕЛАЛ ВИДИМЫМ:
# при полном прогоне порядок всегда один, и зависимость невидима. Ровно тот
# класс, что «работает, потому что множество состоит из одного элемента»
# (`71` §7в).
#
# ✅ **ЗАКРЫТО В ТОТ ЖЕ ДЕНЬ объявлением зависимостей** (`NEEDS_*` выше).
# Фильтр подтягивает опоры сам: `TZONE=A` → 32 проверки, ноль падений
# (было 25/2); `TCASE=A2` → 8 проверок за 26 с, `G1` прогоняется
# автоматически. Все зоны зелёные: C 48, D 30, E 35.
#
# 🔴 **Приёмка по-прежнему полным прогоном.** Объявленная зависимость
# закрывает ИЗВЕСТНЫЕ связи; неизвестная не объявлена по определению,
# и найдётся она только тем, что зона вдруг покраснеет. Зелёная зона
# при фильтре означает «известные опоры построены», а не «зона в порядке».
CASE_ACTIVE=1
SKIPPED=0

# 🔴 ЗАВИСИМОСТИ КЕЙСОВ — объявляются, а не угадываются.
#
# Часть кейсов работает ПОВЕРХ состояния, построенного другим: `A2` проверяет
# идемпотентность того, что опубликовал `G1`, и без него читает пустой каталог.
# Замер 04.09.2026: шесть каталогов делятся между кейсами
# (`c2`: A2 C2 G1 · `t1`: A1 D1 G2 · `t2`: A2 G1 · `t22`: C7 E4 E5 ·
#  `c22`: C7 E5 · `t43`: D5 D6).
#
# 🔴 РАЗВОДИТЬ КАТАЛОГИ БЕССМЫСЛЕННО: зависимость **смысловая**, а не
# случайная. `A2` нужен не каталог `c2`, а факт, что `G1` уже опубликовал
# в него репу. Своё имя каталога ничего не изменит — нужен прогон `G1`.
#
# Поэтому зависимость ОБЪЯВЛЯЕТСЯ строкой ниже, и фильтр подтягивает её сам:
# `TZONE=A` автоматически прогонит и `G1`, от которого зависит `A2`.
# Объявление читается из этого же файла при старте — один источник, не два.
NEEDS_A2="G1"
NEEDS_C2="G1"
NEEDS_D1="A1"
NEEDS_G2="A1"
...
# Раскрытие: к запрошенному набору добавляются его зависимости, транзитивно.
_wanted=""
_want_add(){
  case " $_wanted " in *" $1 "*) return 0 ;; esac
  _wanted="$_wanted $1"
  eval "_dep=\${NEEDS_$1:-}"
  for _d in $_dep; do _want_add "$_d"; done
}
if [ -n "${TZONE:-}${TCASE:-}" ]; then
  for _c in ${TCASE:-}; do _want_add "$_c"; done
  if [ -n "${TZONE:-}" ]; then
    for _c in $(grep -oE '^case_ "[A-Z][0-9]+"' "$0" | sed 's/case_ "//; s/"//'); do
      case "${TZONE}" in *"$(printf '%s' "$_c" | cut -c1)"*) _want_add "$_c" ;; esac
    done
  fi
fi

case_active(){
  [ -z "${TZONE:-}${TCASE:-}" ] && return 0
  case " $_wanted " in *" $1 "*) return 0 ;; esac
  return 1
}
case_(){
  CURRENT="$2"; G_CUR="$(printf '%s' "$1" | cut -c1)"
  if case_active "$1"; then
    CASE_ACTIVE=1
  else
    CASE_ACTIVE=0; SKIPPED=$((SKIPPED+1)); return 0
  fi
...
# 🔴 Отфильтрованный кейс сюда не заходит: здесь ~3 минуты на 122 вызова.
run_deploy(){
  if [ "${CASE_ACTIVE:-1}" = "0" ]; then return 0; fi
  ( cd "$1" && KEEP_ARCHIVES="${KEEP_ARCHIVES:-1}" bash "$DEPLOY" "$1" 2>&1 )
}
...
case_ "H10" "Сообщение MISSING не звучит как блокер — говорит, что шаг 3 чинит сам"
# v4.20.1. Живой прогон: «ТРЕБУЕТ РЕШЕНИЯ: repos-map: нет в карте вовсе — X» напечаталось,
# а следующим шагом скрипт сам добавил строку — противоречие того же класса, что H9 чинит
# для итогового блока, но здесь про inline-сообщение по ходу прогона.
