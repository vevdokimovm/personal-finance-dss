...
2. **WATCHLOG** (`docs/WATCHLOG.md`) — точка передачи. §0 — «отсюда продолжаем», §3 — **ровно 10**
   ...
   отдельным `.md` (с версией в имени: `WATCHLOG-finpilot-vX.Y.Z.md` — по стандарту `43` §1: дефис между
   именем и версией, **точки внутри версии**).
...
3. **Инциденты/расследования** отдаются локально через `present_files`; в архив идёт только
   строка-сводка в реестр (`docs/reports/incidents_summary.md`) — чтобы не раздувать репо (§12).
