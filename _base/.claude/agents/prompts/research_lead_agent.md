# Research Lead Agent Prompt — оригинал Anthropic

> Снято дословно 28.08.2026 через `WebFetch` с `github.com/anthropics/
> claude-cookbooks/blob/main/patterns/agents/prompts/research_lead_agent.md`
> (`anthropic-cookbook`, публичный, официальный). Источник для `researcher.md`
> (`.claude/agents/researcher.md`) — не пересказ, оригинал сохранён рядом,
> чтобы адаптация не разошлась молча с первоисточником.
...
5.  Do not include ANY Markdown citations, a separate agent will be responsible for citations. Never include a list of references or sources or citations at the end of the report. </answer_formatting>
