...
- how-to-use.md
- 00-navigator.md
- context-preferences-edition.md
- context-vasilii-unified.md
- memory-all-accounts.md
- standards-code-career.md
- memory-export-other-ai.md
- health-clinical-portrait.md
- personal-appearance-analysis.md
- migration-2year-plan.md
- walk-with-jesus-christ.md
- education.md
- Agents.md
- CLAUDE.md
- Context.md
...
### `00-core/` — грузить в каждый аккаунт
| Файл | Что это |
|------|---------|
| `context-vasilii-unified.md` | **Главный файл-сингуляр.** Кто я, тон, психология, полный медблок, генетика, когнитивный профиль, вера, код-preferences, проекты, цели, роли, ритуалы. Общие инструкции для Клода. |
| `memory-all-accounts.md` | Сжатая память (то, что Клод «помнит»). Вставлять во все аккаунты. |
| `memory-export-other-ai.md` | Портируемый профиль без привязки к Claude — отдать другому ассистенту (ChatGPT/Gemini и т.п.). |
| `standards-code-career.md` | Техсвод: Python-backend, архитектура, frontend, pre-push, README, поведение при коде, interview-prep. Грузить в рабочие сессии. |

### `01-blocks/` — отдельные логические блоки (по контексту)
| Файл | Что это |
|------|---------|
| `migration-2year-plan.md` | 🎯 **Приоритет №1.** Главный вектор на 2 года: магистратура + миграция. |
| `health-clinical-portrait.md` | Подробный медблок (психика, лабы, генетика, фарма). |
| `education.md` | Весь академ-путь: школа → бакалавриат → магистратура + приоритеты поступления. |
| `personal-appearance-analysis.md` | Приватный фреймворк. **Вне общего контекста**, только точечно. |

### `02-instructions/` — 14 исходных инструкций, обновлённых до актуального
- **Личные (обновлены содержательно):** `CLAUDE.md`, `Memory.md`, `Context.md`, `psychology.md`, `tone-of-voice.md`, `my-character.md`, `interview-prep.md`
...
### `03-project-docs/` — в папку `docs/` проекта (FINPILOT / новые)
...
1. **В каждый аккаунт Claude** (Project knowledge): весь кит, минимум `context-vasilii-unified.md` + `memory-all-accounts.md`. Это убирает расхождение памяти между аккаунтами.
2. **В рабочие/кодовые сессии** — добавлять `standards-code-career.md` и нужные блоки из `01-blocks/`.
3. **Файлы `02-instructions/`** — заменяют старые версии в Cowork/Projects (система `@import` из `CLAUDE.md` сохранена).
4. **`03-project-docs/`** — в `docs/` соответствующего репо (часть портфолио, рекрутер видит зрелость).
...
======================================================================
# ФАЙЛ: context-preferences-edition.md
======================================================================
...
======================================================================
# ФАЙЛ: context-vasilii-unified.md
======================================================================
...
======================================================================
# ФАЙЛ: memory-export-other-ai.md
======================================================================
...
======================================================================
# ФАЙЛ: health-clinical-portrait.md
======================================================================
...
======================================================================
# ФАЙЛ: personal-appearance-analysis.md
======================================================================

# ЛИЧНОЕ — фреймворк анализа внешности

> ⚠️ **Приватный файл. НЕ входит в общий контекст.** Подключать только точечно, когда я сам прошу такой анализ. В `context-vasilii-unified.md` этого нет намеренно.
...
======================================================================
# ФАЙЛ: walk-with-jesus-christ.md
======================================================================
...
- *Тревога/депрессия/леность — это грех?* → тревога и уныние — не «преступление», а **страсти/немощи** (почва — маловерие); леность — да, страсть праздности. С ними борются не виной, а трудом, молитвой и упованием. Важно: F41.2 — это ещё и болезнь, лечение не противоречит вере (см. `health-clinical-portrait.md`).
...
- Магистратура — **часть 2-летнего миграционного плана** (диплom как актив для визы + время на язык/портфолио/накопления). См. `migration-2year-plan.md`.
...
- **Миграция из России + магистратура.** Уехать через работу (приоритет — релокация компанией). Страны: Скандинавия (Швеция — там **брат**-айтишник, Дания, Финляндия, Норвегия, Исландия), Канада?, Япония?, Швейцария, Гренландия (мечта). Мага — скорее всего МИСИС (общага в Москве). Критический путь: IELTS 7.0+, IT-работа сейчас, LinkedIn, загранпаспорт, нотариальные переводы дипломов. См. `migration-2year-plan.md`.
...
- Подробности — `health-clinical-portrait.md`
...
**🎯 Главный план на 2 года:** миграция из России + магистратура (скорее всего МИСИС, общага в Москве). Уехать через работу — приоритет оффер + релокация компанией (так уехал **брат** в Швецию ~2019 через LinkedIn). Страны: Скандинавия/Швеция, Дания, Финляндия, Норвегия, Исландия, Канада?, Япония?, Швейцария, Гренландия (мечта). См. `migration-2year-plan.md`.
...
**База крепкая** (печень, почки, щитовидка, метаболизм, тестостерон, гемостаз, ИППП — норма; МРТ мозга чистая). Подробно — `health-clinical-portrait.md`.
...
ООП, строгий PEP 8, type hints только на сигнатурах, минимум комментариев, обработка ошибок где критично, зависимости по скорости, тесты по запросу, секреты через `.env`, Python 3 only, без глобальных. Новый код → полная реализация → короткое объяснение; рефакторинг → полный переписанный; архитектура → 2–3 варианта с trade-offs; код в чате + файл опцией. Не предлагать: мобилку, геймдев, «погугли». Полный техсвод — `standards-code-career.md`.
