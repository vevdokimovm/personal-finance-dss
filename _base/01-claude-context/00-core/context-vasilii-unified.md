...
> Синхронизирован из аккаунтов (**V Vasilii / S Sergey / J Jesus / M Michael / A Adam**)
> + инструкций + анализа всех чатов за всё время.
> **Актуальность: 23.08.2026.** Вахт пять — пятая (A, Adam) заведена 23.08.2026.
...
## 9. Текущий контекст и проекты (август 2026)
...
- Зрелость: **v8.28.0**, идёт **веха 8** — вынести на SPA всё, что висит только в API (уведомления, реферальная, админ-аналитика, история планов, households, A/B, экспорт), CRUD-батч 3 «Бюджет», юр-требования L2–L7, тёмная тема и mobile-адаптив. Дальше по плану: Э6 юрконтур + **снос Jinja-остатка**, Э7 безопасность (CSP на nonce, токены, перф-бюджеты), §8.4 PDF-отчёт, §8.3 онбординг, Э8 закрытие.
- 🔴 **Отдельное поручение владельца, §9.0 E:** класс тестов «полный эмпирический путь реального пользователя» — реальный HTTP + реальный браузер, **не моки**. Сейчас такого в дереве нет: блокирующий `fast` гоняет e2e против легаси-Jinja, фронтовые Playwright-спеки целиком на моках.
- Сотни тестов (fast/full/deep), CI с гейтами (coverage ≥90%, flake8=0, revision-gate), **TDD обязателен**. Матмодель — канон **v3.0.0**: SAW + Avalanche + SES+Монте-Карло, 5 риск-профилей, 66 альтернатив (шаг 10%), инварианты Rt≥0 и ПДН≤0.40. **v2.x запрещена.**

**Магистратура — поступление состоялось.** МИСИС, **09.04.01 «Информатика и вычислительная техника»**, очная, бюджет.

| | |
|---|---|
| позиция в конкурсном списке | **15** (срез 16.08.2026) |
| сумма баллов | **113** = 100 за ВИ + 13 ИД |
| проходной на 17.08 | **103** — запас **+10** |
| мест / заявлений | 65 / ≈910 (**≈14 на место**) |
| согласие на зачисление | подано, электронное |
| код поступающего | `1077353`, приоритет 1 |

🔴 **Урок конкурса:** восемь человек со **100 баллами за испытание** не проходят на бюджет, потому что у них ноль ИД. При потолке в 100 за ВИ единственная ось, по которой ещё можно двигаться, — **индивидуальные достижения**.

**Загранпаспорт:** заявление на новый (нового образца) подано через Госуслуги **17.08.2026**, ОВМ УМВД Подольск, срок 1 месяц. Действующий 76 1437484 (до 24.08.2029) при выдаче нового **аннулируется**. В подразделение просят военный билет — узловой пункт для призывной ситуации.

**Экосистема репозиториев-баз знаний (`repos-map`) — сквозной активный инструмент.** Это НЕ архив-свалка, а **живая система активных конспектов и продуктивных инструментов обучения**: одна репа = одна зона жизни, всё поддерживается в актуальном состоянии, тяжёлые файлы сжимаются в тезисные `.md` (суть без воды). С этими репами мы **активно работаем и улучшаем их** — это рабочая среда, не хранилище. 📍 **На 23.08.2026 в системе 61 репозиторий** (в июле было 11). Ядро: `it-base` (IT/карьера), `edu-base` (самообучение), `academic-portfolio` (формальный путь), `truth-seeking` (критическое мышление), `christ-walk` (вера), `family` (генеалогия), `misc-vault` (разное без своей зоны), `self-map` (изучение себя + дневник), `health-vault` (здоровье), `legal-knowledge-base` (юриспруденция), `portrait-of-taste`, `master-admission`, `exam-kit`, `dota-dossier`, `research-craft`, `ai-relay`. Приватные все, кроме восьми публичных витрин.

**Кампании 2026:**

- 🏁 **Синтез — закрыт 22.08.2026.** Все 57 репозиториев пройдены, 57 приёмок, ноль пропусков; стандарт выведен (`05-infra-synthesis-lab/STANDARD.md` §00).
- 🏁 **Перенос `~/Documents` → репы — закрыт 22.08.2026.** ~43 500 файлов, `~/Documents` 7.4 → **5.8 ГБ**, 24 папки-источника → 0.
- 🔄 **Ревизия — идёт.** Не механика, а чтение содержимого: манифесты (11 из 61), перевод медиа в текст, дедуп, мёртвые пути (183 → 8), классы реп (61/61).
- 🔴 **Найдено владельцем 23.08:** ревизия подменялась механикой, потому что механика отчитывается числами. Замер: 5 236 медиа-файлов, выжимку имели **23 (0.4 %)**. Заведена лаборатория `07-media-to-text-lab`; выжимок стало **543**, для изображений выведен отдельный метод (описание содержания, а не сжатие).
...
Я работаю над FINPILOT с пяти аккаунтов и передаю «вахту» через **`docs/WATCHLOG.md`** (вахтенный журнал внутри репо). Канон: **«один zip = полный контекст»** — вся теория, стандарты и матмодель живут внутри продукта (репо/архив), это единственный источник правды.
